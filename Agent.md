...
- PR #168 merged to `main` at `a1f2f05` and owns Petty Cash Voucher issuance plus its cash-advance source integration. Its changed `PAY-DED-001` and `PAY-LOAN-001` scope remains `IMPLEMENTED` in the ledger until linked exact-head verification is recorded. Generic payroll schedules must preserve the Petty Cash ledger/materializer and must not mutate the voucher ledger or loan balance during calculation.
- `PAY-TASK-011` on draft PR #171 (`codex/payroll-schedules-controls-latest-20260824`) adds effective-dated `ONE_TIME`, `RECURRING`, and `UNTIL_FULLY_PAID` generic deduction schedules; loan-installment materialization; tenant/employee statutory applicability versions; a six-task admin payroll workspace; and employee schedule/statutory visibility. Its new requirements are `IMPLEMENTED` locally and remain pending exact-head CI, review, merge, and separate deployment confirmation. Do not report them as production merely because the earlier payroll baseline is deployed.
- PR #167's billing-search/homeowner-selection work is merged and remains outside this payroll task. PR #170 owns receipt browser-print changes and is also outside payroll scope.
