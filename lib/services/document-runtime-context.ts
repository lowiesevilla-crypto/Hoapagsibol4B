...
export function permissionsForRole(role: Role): readonly DocumentPermission[] {
  ...
  if (role === Role.PAYROLL_MANAGER) return ["VIEW_DEFINITIONS", "VIEW_TEMPLATES"];
  if ([Role.ADMIN, Role.SYSTEM_ADMIN, Role.HOA_ADMIN, Role.SUPER_ADMIN, Role.PLATFORM_ADMIN].includes(role)) return [
    ...
    "REVOKE_DOCUMENT", "RELEASE_DOCUMENT", "VIEW_ISSUED_DOCUMENT", "VIEW_GENERATION_HISTORY", "RETRY_GENERATION",
  ];
  return [];
}
