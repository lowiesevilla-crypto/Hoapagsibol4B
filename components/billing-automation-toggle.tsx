...
export function BillingAutomationToggle({ defaultAutomatic, defaultBillingDay, modeError, dayError }: { defaultAutomatic: boolean; defaultBillingDay: number; modeError?: string; dayError?: string }) {
  ...
  const [billingDay, setBillingDay] = useState(String(defaultBillingDay));

  useEffect(() => {
    setAutomatic(defaultAutomatic);
    setBillingDay(String(defaultBillingDay));
  }, [defaultAutomatic, defaultBillingDay]);

  const displayBillingDay = Number(billingDay) || 1;
  ...
  return <div className="md:col-span-2 xl:col-span-2 rounded-2xl border border-pine-100 bg-pine-50/50 p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-[180px_1fr] sm:items-end">
      <label className="label">Monthly Dues billing day<input className="field mt-1" name="billingDay" type="number" min="1" max="28" value={billingDay} onChange={(event) => setBillingDay(event.target.value)} required /></label>
      <div className={`rounded-xl px-3 py-2 text-xs font-bold ${automatic ? "bg-emerald-50 text-emerald-800" : "bg-slate-100 text-slate-500"}`}>{automatic ? `ON · Monthly Dues will run on day ${displayBillingDay}; rental agreements use their own billing day.` : "OFF · Billing remains manual until this switch is enabled."}</div>
    </div>
