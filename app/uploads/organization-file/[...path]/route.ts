...
export async function GET(_request: Request, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (!new Set(["photos", "signatures"]).has(relative[0])) return new Response("Invalid image path.", { status: 400 });
  try {
    const filePath = tenantScoped ? await locateTenantUpload(user.tenant.slug, "organization", ...relative) : path.join(uploadDirectory("organization"), ...relative);
    ...
    return new Response(new Uint8Array(await readFile(filePath)), { headers: { "Content-Type": contentType, "Content-Length": String(info.size), "Cache-Control": "private, max-age=3600", "X-Content-Type-Options": "nosniff" } });
  } catch { return new Response("Image not found.", { status: 404 }); }
}

function contentTypeFor(filePath: string) { return ({ ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png", ".webp": "image/webp" } as Record<string, string>)[path.extname(filePath).toLowerCase()] || ""; }
