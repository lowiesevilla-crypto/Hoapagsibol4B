...
const billingJobItemBatchSize = 250;
...
export async function createBillingGenerationJob(input: BillingGenerationInput, idempotencyKey: string) {
  ...
  const targetRows = preview.rows.filter((row) => row.action === "CREATE");
  const jobId = cryptoSafeCuidLikeId();
  const reference = billingJobReference(input.coverageYear, input.coverageMonth);
  const terminal = targetRows.length === 0;
    ...
    const job = await prisma.$transaction(async (tx) => {
      const created = await tx.billingGenerationJob.create({
        data: {
          ...
          failed: 0,
          skipped: 0,
          status: terminal ? BillingGenerationJobStatus.SUCCEEDED : BillingGenerationJobStatus.QUEUED,
      ...
      for (let index = 0; index < targetRows.length; index += billingJobInsertBatchSize) {
        const rows = targetRows.slice(index, index + billingJobInsertBatchSize);
...
export async function createFailedBillingGenerationRetry(input: {
  ...
  if (![BillingGenerationJobStatus.PARTIAL, BillingGenerationJobStatus.FAILED].includes(source.status)) {
    throw new Error("Only a completed billing job with failed records can be retried.");
  }
...
export async function processBillingGenerationJob(jobId: string, actor: BillingGenerationJobActor) {
  ...
  const now = new Date();
  const leaseExpiresAt = new Date(now.getTime() + billingJobLeaseMs);
  ...
  try {
    while (true) {
      ...
      try {
        ...
        for (const item of pending) {
          ...
          } else if (row.action === "CREATE" && row.billId) {
    ...
    try {
      await prisma.billingGenerationJobItem.updateMany({
        where: { tenantId: actor.tenantId, jobId, status: BillingGenerationJobItemStatus.PENDING },
        data: { status: BillingGenerationJobItemStatus.FAILED, attempts: { increment: 1 }, message },
      });
      ...
      return await syncBillingGenerationJobCounts(jobId, actor.tenantId, leaseToken, true);
    } catch {
      await prisma.billingGenerationJob.updateMany({
        where: { id: jobId, tenantId: actor.tenantId, leaseToken },
        data: {
...
export async function getBillingGenerationJobView(jobId: string, tenantId: string): Promise<BillingGenerationJobView | null> {
  ...
  if (!job) return null;
  return serializeBillingJob(job);
}
...
async function updateBillingJobItems(
  ...
  await prisma.$transaction([
    ...
    prisma.billingGenerationJobItem.updateMany({
      ...
      data: { status: BillingGenerationJobItemStatus.SKIPPED, attempts: { increment: 1 }, message: "Skipped because the billing state changed after preview." },
    }),
    prisma.billingGenerationJobItem.updateMany({
...
async function syncBillingGenerationJobCounts(jobId: string, tenantId: string, leaseToken: string, forceTerminal = false) {
  ...
  const status = terminal
    ? failed === 0
      ? BillingGenerationJobStatus.SUCCEEDED
      : succeeded + skipped > 0
        ? BillingGenerationJobStatus.PARTIAL
        : BillingGenerationJobStatus.FAILED
    : BillingGenerationJobStatus.RUNNING;
...
function cryptoSafeCuidLikeId() {
  return `bgj_${randomUUID().replaceAll("-", "")}`;
}
