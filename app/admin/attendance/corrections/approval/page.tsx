...
export default async function AttendanceCorrectionApprovalPage() {
  ...
  return <>
    ...
    <form action={bulkReviewAttendanceAdjustmentsAction} className="card">
      ...
      <StandardTable><div className="table-wrap shadow-none"><table className="data-table"><thead><tr><th></th><th>Employee</th><th>Attendance date</th><th>Current time</th><th>Requested time</th><th>Reason</th><th>Status</th><th>Requested by</th></tr></thead><tbody>{requests.map((item) => {
      ...
      })}{!requests.length && <tr><td colSpan={8} className="py-12 text-center text-slate-500">No correction requests yet.</td></tr>}</tbody></table></div></StandardTable>
    </form>
  </>;
}
