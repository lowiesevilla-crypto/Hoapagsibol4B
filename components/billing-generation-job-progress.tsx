...
const terminalStatuses = new Set<BillingJobView["status"]>(["SUCCEEDED", "PARTIAL", "FAILED"]);

export function BillingGenerationJobProgress({ jobId, processingEnabled }: { jobId: string; processingEnabled: boolean }) {
  const router = useRouter();
  const [job, setJob] = useState<BillingJobView | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const payload = await response.json() as BillingJobView | { error?: string };
        if (!response.ok || !("status" in payload)) {
          throw new Error("error" in payload && payload.error ? payload.error : "Billing progress could not be loaded.");
        }
        if (cancelled) return;
        setJob(payload);
        setError("");
        ...
        pollTimer = setTimeout(load, 4_000);
      }
    }

    void resume();
    void load();
    if (processingEnabled) resumeTimer = setInterval(() => void resume(), 15_000);
    return () => {
      cancelled = true;
      if (pollTimer) clearTimeout(pollTimer);
      if (resumeTimer) clearInterval(resumeTimer);
    };
  }, [jobId, processingEnabled, router]);

  async function retryFailed() {
    if (!job || retrying || !processingEnabled) return;
    setRetrying(true);
    setError("");
  ...
  return <section className="card mb-6" aria-labelledby="billing-job-title">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>
        <p className="text-xs font-black uppercase tracking-wider text-pine-700">Durable monthly billing job</p>
        <h2 id="billing-job-title" className="mt-1 text-lg font-black">{job.reference}</h2>
        <p className="mt-1 text-sm text-slate-500">{month} · {job.scope.replaceAll("_", " ")} · Progress is persisted and can be reopened after refresh, reconnection, or navigation.</p>
      </div>
    ...
    <div className="mt-5" aria-live="polite">
      ...
      <p className="mt-2 text-xs text-slate-500">Percentage = floor(completed records ÷ total target records × 100). No simulated time-based progress is used.</p>
    </div>

    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <ProgressStat label="Succeeded" value={job.succeeded} />
      <ProgressStat label="Failed" value={job.failed} />
      <ProgressStat label="Skipped" value={job.skipped} />
    ...
    {job.canRetry && <div className="mt-5 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-slate-500">Retry creates a new job containing only failed homeowner records. Successful and skipped records are never resubmitted by this control.</p>
      <button type="button" className="btn-secondary min-h-10 shrink-0 px-4" disabled={retrying || !processingEnabled} onClick={retryFailed}>
        {retrying ? <LoaderCircle className="size-4 animate-spin motion-reduce:animate-none" /> : <RotateCcw className="size-4" />}
        {processingEnabled ? retrying ? "Starting retry…" : `Retry ${job.failed.toLocaleString()} failed` : "Retry disabled by rollout"}
      </button>
    </div>}
  </section>;
}
...
function statusLabel(status: BillingJobView["status"]) {
