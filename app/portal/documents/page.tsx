...
export default async function PortalDocumentsPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; message?: string; status?: string; type?: string; date?: string; page?: string }> }) {
  ...
  const [requests, requestCount, unpaid, paymentSettings, configs, members] = await Promise.all([
    prisma.documentRequest.findMany({ where, include: { histories: { include: { actor: true }, orderBy: { createdAt: "desc" } }, configuration: true, definition: true }, orderBy: { requestedAt: "desc" }, skip: (page - 1) * 10, take: 10 }),
  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
      <div className="space-y-6">
        <DocumentRequestForm configs={portalConfigs} members={activeMembers} />
      ...
      <section className="card"><h2 className="text-lg font-black">My request history</h2><p className="mb-4 text-sm text-slate-500">Status changes and generated documents remain available here.</p>
        {requests.length === 0 ? <p className="rounded-2xl bg-slate-50 p-8 text-center text-sm text-slate-500">You have not submitted a document request yet.</p> : <div className="space-y-3">{requests.map((item) => { const subject = snapshotRecord(item.subjectSnapshot); const downloadable = item.generatedContent && isReadyForDownload(item.status) && (unpaidBalance <= 0 || item.allowDownloadDespiteBalance) && !item.paymentRequiredSnapshot; return <article key={item.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4"><div className="flex flex-col justify-between gap-2 sm:flex-row"><div><p className="font-black">{item.definition?.displayName || item.configuration?.displayName || documentTypeLabel(item.type)}</p><p className="text-xs text-slate-500">{item.origin === "ADMIN" ? "Created by HOA office" : `Requested ${shortDate(item.requestedAt)}`}{item.documentNumber ? ` | ${item.documentNumber}` : ""}{item.generatedAt ? ` | Generated ${shortDate(item.generatedAt)}` : ""}</p></div><span className={`badge ${isReadyForDownload(item.status) ? "badge-paid" : item.status === "REJECTED" ? "badge-overdue" : "badge-info"}`}>{item.status.replaceAll("_", " ")}</span></div><p className="mt-2 text-sm text-slate-600">{item.purpose}</p><p className="mt-1 text-xs font-bold text-slate-500">Subject: {String(subject.fullName || "Registered homeowner")}{subject.relationship ? ` (${String(subject.relationship)})` : ""} | Fee: {money(Number(item.feeAmountSnapshot))}</p>{(item.adminRemarks || item.remarks) && <p className="mt-2 rounded-xl bg-white p-2 text-xs"><b>Remarks:</b> {item.adminRemarks || item.remarks}</p>}{item.paymentRequiredSnapshot && <p className="mt-2 rounded-xl bg-amber-100 p-2 text-xs font-bold text-amber-900">Payment confirmation is required before download. Finance collection integration is scheduled separately.</p>}{item.generatedContent && <div className="mt-3 flex flex-wrap gap-2"><Link className="btn-secondary min-h-9 px-3 py-1.5 text-xs" href={`/documents/${item.id}`}>View details</Link>{downloadable ? <><a className="btn-primary min-h-9 px-3 py-1.5 text-xs" href={`/documents/${item.id}/pdf`}>Download PDF</a><Link className="btn-secondary min-h-9 px-3 py-1.5 text-xs" href={`/documents/${item.id}/print`}>Print</Link></> : <span className="rounded-xl bg-amber-100 px-3 py-2 text-xs font-bold text-amber-900">Download locked</span>}</div>}<details className="mt-3 rounded-xl bg-white p-3 text-xs"><summary className="cursor-pointer font-bold">Status history ({item.histories.length})</summary><div className="mt-2 space-y-1">{item.histories.map((history) => <p key={history.id}><b>{history.status.replaceAll("_", " ")}</b> - {shortDate(history.createdAt)}{history.note ? `: ${history.note}` : ""}</p>)}</div></details></article>; })}</div>}
