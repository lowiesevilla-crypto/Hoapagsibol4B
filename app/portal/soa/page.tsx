...
export default async function PortalStatementOfAccountPage() {
  ...
  return <PortalPageContainer className="space-y-4 sm:space-y-5">
    ...
    <div className="print-hidden space-y-4 sm:space-y-5">
      ...
      <PageHeader eyebrow="My account" title="Statement of Account" description="Your current account position, billing activity, and payment history." action={<div className="flex flex-wrap gap-2"><Link className="btn-secondary" href="/portal/pay"><QrCode className="size-4" /> Pay dues</Link><SoaPrintButton /></div>} />
      ...
      <SoaDisclosure title="Receivables aging" eyebrow="Aging" summary={`${money(totalAging(soa.aging))} receivable`}>
...
function SoaDisclosure({ title, eyebrow, summary, children }: { title: string; eyebrow: string; summary: string; children: React.ReactNode }) {
  return <details className="soa-disclosure group overflow-hidden rounded-2xl border border-pine-100 bg-white shadow-soft">
    <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-4 py-3 focus-visible:outline focus-visible:outline-2 focus-visible:outline-pine-500 [&::-webkit-details-marker]:hidden">
