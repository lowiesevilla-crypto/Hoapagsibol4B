...
export async function createPayMongoInvoiceCheckout(invoiceId: string) {
  ...
  const response = await fetch(CHECKOUT_ENDPOINT, {
    ...
    body: JSON.stringify({
      data: {
        attributes: {
          line_items: [{
            name: `${invoice.subscription.plan.name} · ${invoice.invoiceNumber}`,
            description: `HOAHub subscription invoice ${invoice.invoiceNumber}`,
            amount: cents,
  ...
  const existing = await prisma.platformPayment.findFirst({
    where: { gateway: PlatformPaymentGateway.PAYMONGO, gatewayCheckoutId: checkoutId },
  });
  if (!existing) {
...
async function beginGatewayEvent(
  payload: unknown,
  rawBody: string,
  event: ReturnType<typeof normalizeCheckoutEvent>,
) {
  const providerEventId = event.explicitEventId
    || `evt_hash_${createHash("sha256").update(rawBody).digest("hex")}`;
  const existing = await prisma.platformGatewayEvent.findUnique({
    where: {
      provider_providerEventId: {
        provider: PlatformPaymentGateway.PAYMONGO,
        providerEventId,
      },
    },
  });
  if (existing && [PlatformGatewayEventStatus.PROCESSED, PlatformGatewayEventStatus.IGNORED].includes(existing.status)) {
    return { eventRecord: existing, complete: true as const };
  }
  if (existing) {
    return {
      eventRecord: await prisma.platformGatewayEvent.update({
        ...
        data: {
          ...
          status: PlatformGatewayEventStatus.RECEIVED,
  ...
  return {
    eventRecord: await prisma.platformGatewayEvent.create({
      data: {
        provider: PlatformPaymentGateway.PAYMONGO,
        providerEventId,
        eventType: event.eventType,
...
export async function processPayMongoWebhook(rawBody: string, signatureHeader: string | null) {
  const verification = verifyPayMongoWebhookSignature(
    rawBody,
    signatureHeader,
    requiredSecret("PAYMONGO_WEBHOOK_SECRET"),
  );
  if (!verification.valid) {
    return { ok: false as const, status: 401, message: "Invalid PayMongo signature." };
  }
  ...
  const started = await beginGatewayEvent(payload, rawBody, event);
  if (started.complete) {
    return { ok: true as const, duplicate: true, eventId: started.eventRecord.id };
  }
  const providerEvent = started.eventRecord;

  if (event.eventType !== "checkout_session.payment.paid") {
    await prisma.platformGatewayEvent.update({
      where: { id: providerEvent.id },
      data: { status: PlatformGatewayEventStatus.IGNORED, processedAt: new Date() },
    });
  ...
  const paidPayment = payments.find((item) =>
    String((item.attributes as Record<string, unknown> | undefined)?.status || "").toLowerCase() === "paid",
  ) || payments[0];
  ...
  try {
    ...
    const alreadyPosted = await prisma.platformPayment.findFirst({
      where: {
        tenantId: invoice.tenantId,
        gateway: PlatformPaymentGateway.PAYMONGO,
        gatewayPaymentId,
    ...
    if (alreadyPosted) {
      await prisma.platformGatewayEvent.update({
        ...
        data: {
          tenantId: invoice.tenantId,
          status: PlatformGatewayEventStatus.PROCESSED,
          processingError: null,
          processedAt: new Date(),
        },
      });
      return { ok: true as const, duplicate: true, eventId: providerEvent.id, invoiceId: invoice.id };
    }

    if (Math.abs(Number(invoice.outstandingBalance) - paidAmount) > 0.009) {
      throw new Error("PayMongo amount does not match the current outstanding invoice balance.");
    }
    ...
    await prisma.$transaction(async (tx) => {
      const payment = pending
        ? await tx.platformPayment.update({
            ...
            data: {
              ...
              status: PlatformPaymentStatus.SUCCEEDED,
              paidAt,
              metadata: { invoiceId: invoice.id, invoiceNumber: invoice.invoiceNumber, checkoutId },
            },
          })
      ...
      await tx.platformPaymentAllocation.upsert({
      ...
      await tx.platformGatewayEvent.update({
        where: { id: providerEvent.id },
        data: {
          tenantId: invoice.tenantId,
          status: PlatformGatewayEventStatus.PROCESSED,
    ...
    await prisma.platformGatewayEvent.update({
      where: { id: providerEvent.id },
      data: {
        status: PlatformGatewayEventStatus.FAILED,
