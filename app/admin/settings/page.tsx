...
export default async function SystemSettingsPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; message?: string }> }) {
  ...
  return <>
    <PageHeader eyebrow="System administration" title="Configuration center" description="Manage connection strings, email delivery, Facebook Page tokens, GCash QR payment settings, and association records." action={<div className="flex flex-wrap gap-2"><Link className="btn-secondary" href="/admin/documents">Document management</Link><Link className="btn-primary" href="/admin/settings/organization">Manage organization</Link></div>} />
