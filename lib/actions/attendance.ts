...
import { AttendanceAdjustmentStatus, AttendanceStatus, NotificationChannel, NotificationStatus, NotificationType, PayrollDayType, PayrollStatus } from "@prisma/client";
...
export async function saveAttendanceAction(formData: FormData) {
  ...
  if (id) {
    ...
    await assertAttendanceEditable(existing.employeeId, existing.date);
    const duplicate = await prisma.attendance.findFirst({ where: { employeeId: data.employeeId, date: targetDate, id: { not: id } } });
    ...
    const duplicate = await prisma.attendance.findUnique({ where: { employeeId_date: { employeeId: data.employeeId, date: targetDate } } });
    if (duplicate) throw new Error("Attendance already exists for this employee and date. Use Edit to update the existing record.");
    const record = await prisma.attendance.create({ data: values });
...
export async function deleteAttendanceAction(formData: FormData) {
  ...
  const paidPeriod = await prisma.payrollPeriod.findFirst({
    where: { status: PayrollStatus.PAID, startDate: { lte: record.date }, endDate: { gte: record.date }, payslips: { some: { employeeId: record.employeeId } } },
    select: { id: true, startDate: true, endDate: true, payDate: true },
  });
  if (paidPeriod) {
    await requirePayrollAccess(payrollManageRoles);
    const confirmation = String(formData.get("confirmation") || "").trim().toUpperCase();
    const acknowledged = formData.get("acknowledged") === "on";
    if (confirmation !== "DELETE" || !acknowledged) throw new Error("This attendance record is already paid. Acknowledge the warning and type DELETE before continuing.");
  }
  ...
  await prisma.$transaction(async (tx) => {
    await tx.auditLog.create({
      data: {
        actorId: user.id,
        module: "ATTENDANCE",
        action: paidPeriod ? "ARCHIVE_AND_DELETE_PAID_ATTENDANCE" : "DELETE_ATTENDANCE",
        entityType: "Attendance",
        entityId: id,
        metadata: { record: snapshot, paidPeriod: paidPeriod ? attendanceSnapshot(paidPeriod) : null },
      },
    });
  ...
  revalidateAttendancePages();
  revalidatePath("/admin/payroll");
  redirect(`/admin/attendance/history?success=deleted${paidPeriod ? "&paid=1" : ""}`);
}
...
export async function employeeClockInAction(formData: FormData) {
  ...
  const existing = await prisma.attendance.findUnique({ where: { employeeId_date: { employeeId: user.employeeProfile.id, date } } });
  if (existing) await assertAttendanceEditable(existing.employeeId, existing.date);
  ...
  const record = await prisma.attendance.upsert({
    ...
    create: { employeeId: user.employeeProfile.id, date, timeIn, status: AttendanceStatus.PRESENT, remarks, ...metrics },
    update: { timeIn, status: AttendanceStatus.PRESENT, remarks, ...metrics },
  });
...
export async function requestAttendanceCorrectionAction(formData: FormData) {
  ...
  const attendance = await prisma.attendance.findUnique({ where: { employeeId_date: { employeeId: user.employeeProfile.id, date: targetDate } } });
  ...
  await assertAttendanceEditable(attendance.employeeId, attendance.date);
...
export async function reviewAttendanceAdjustmentAction(formData: FormData) {
  ...
  const adjustment = await applyAttendanceAdjustmentReview(parsed.data.id, parsed.data.decision, user.id, parsed.data.reviewRemarks);
...
export async function bulkReviewAttendanceAdjustmentsAction(formData: FormData) {
  ...
  for (const id of ids) {
    const adjustment = await applyAttendanceAdjustmentReview(id, decision, user.id, reviewRemarks);
...
async function applyAttendanceAdjustmentReview(id: string, decision: "APPROVED" | "REJECTED", reviewedById: string, reviewRemarks?: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (locked) throw new Error("This attendance record is already included in a paid payroll period and cannot be changed directly.");
...
async function assertAttendanceEditable(employeeId: string, date: Date) {
  const locked = await prisma.payrollPeriod.findFirst({
    where: { status: PayrollStatus.PAID, startDate: { lte: date }, endDate: { gte: date }, payslips: { some: { employeeId } } },
    select: { payDate: true },
  });
  if (locked) throw new Error("This attendance record is already included in a paid payroll period. Use an authorized payroll adjustment process instead.");
}
