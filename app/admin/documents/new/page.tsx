...
export default async function NewAdminDocumentPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <><PageHeader eyebrow="Resident services" title="Create Walk-In / Office Request" description="Create a tenant-scoped document request for an office-assisted transaction. The required steps depend on the selected document workflow, payment policy, and approval policy." action={<Link className="btn-secondary" href="/admin/documents">Back to Document Management</Link>} />{query.error && <div className="mb-5 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800">{query.error}</div>}<ManualDocumentForm homeowners={homeowners.map((item) => ({ id: item.id, label: `${item.user.name} - Block ${item.block}, Lot ${item.lot}` }))} officers={officers.map((item) => ({ id: item.id, label: `${item.fullName} - ${item.position}` }))} definitions={choices} /></>;
}
