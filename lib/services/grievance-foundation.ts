...
type VerificationRow = { id: string; required: number | boolean; blocksEnforcement: number | boolean; status: ComplaintVerificationStatus };
type GrievanceRow = { id: string; tenantId: string; complaintId: string; status: string; boardReviewRequired: number | boolean };
...
async function audit(input: { tenantId: string; actorId: string | null; action: string; entityType: string; entityId: string; metadata?: Prisma.InputJsonValue }) {
  await platformPrisma.auditLog.create({ data: { tenantId: input.tenantId, actorId: input.actorId, module: "COMPLAINTS", action: input.action, entityType: input.entityType, entityId: input.entityId, metadata: input.metadata } });
}
...
export async function getActiveGrievancePermissions(tenantId: string, userId: string) {
  const rows = await platformPrisma.$queryRaw<PermissionRow[]>`
    SELECT permissions FROM GrievanceCommitteeMembership
    WHERE tenantId = ${tenantId} AND userId = ${userId} AND active = true
      AND startsAt <= NOW(3) AND (endsAt IS NULL OR endsAt > NOW(3))
  `;
...
export async function addComplaintSubject(user: EffectiveUser, input: { complaintId: string; subjectType: ComplaintSubjectType; homeownerId?: string | null; vehicleId?: string | null; displayLabel?: string | null }) {
  ...
  const complaint = await platformPrisma.complaint.findFirst({ where: { tenantId: user.tenantId, id: input.complaintId }, select: { id: true } });
  ...
  if (input.homeownerId) {
    homeowner = await platformPrisma.homeownerProfile.findFirst({ where: { tenantId: user.tenantId, id: input.homeownerId }, select: { id: true, phase: true, block: true, lot: true, address: true } });
    if (!homeowner) throw new Error("The selected property subject is not available in this HOA.");
  }
  if (input.vehicleId) {
    vehicle = await platformPrisma.vehicle.findFirst({ where: { tenantId: user.tenantId, id: input.vehicleId }, select: { id: true, homeownerId: true, plateNumber: true } });
    if (!vehicle) throw new Error("The selected vehicle subject is not available in this HOA.");
    if (homeowner && homeowner.id !== vehicle.homeownerId) throw new Error("The selected vehicle does not belong to the selected homeowner/property.");
    if (!homeowner) homeowner = await platformPrisma.homeownerProfile.findFirst({ where: { tenantId: user.tenantId, id: vehicle.homeownerId }, select: { id: true, phase: true, block: true, lot: true, address: true } });
  }
  if ((input.subjectType === "HOMEOWNER" || input.subjectType === "PROPERTY") && !homeowner) throw new Error("Select a same-HOA homeowner/property for this subject.");
...
export async function upsertComplaintVerificationPolicy(user: EffectiveUser, input: { policyKey: string; categoryId?: string | null; privacyMode?: ComplaintPrivacyMode | null; verificationRequired: boolean; blocksEnforcement: boolean; active?: boolean }) {
  ...
  await platformPrisma.$executeRaw`
    ...
    ON DUPLICATE KEY UPDATE categoryId = VALUES(categoryId), privacyMode = VALUES(privacyMode), verificationRequired = VALUES(verificationRequired), blocksEnforcement = VALUES(blocksEnforcement), active = VALUES(active), updatedById = VALUES(updatedById), updatedAt = NOW(3)
  `;
...
export async function evaluateComplaintVerificationRequirement(tenantId: string, complaintId: string) {
  const complaint = await platformPrisma.complaint.findFirst({ where: { tenantId, id: complaintId }, select: { id: true, categoryId: true, privacyMode: true } });
  if (!complaint) throw new Error("Complaint not found.");
  const policies = await platformPrisma.$queryRaw<Array<{ verificationRequired: number | boolean; blocksEnforcement: number | boolean }>>`
    SELECT verificationRequired, blocksEnforcement FROM ComplaintVerificationPolicy
  ...
  const id = randomUUID();
  await platformPrisma.$executeRaw`
    INSERT INTO ComplaintVerification (id, tenantId, complaintId, required, blocksEnforcement, status, createdAt, updatedAt)
    VALUES (${id}, ${tenantId}, ${complaint.id}, ${required}, ${blocksEnforcement}, ${required ? "PENDING" : "NOT_REQUIRED"}, NOW(3), NOW(3))
    ON DUPLICATE KEY UPDATE
      required = VALUES(required), blocksEnforcement = VALUES(blocksEnforcement),
      status = CASE WHEN status = 'PASSED' THEN status WHEN VALUES(required) = false THEN 'NOT_REQUIRED' WHEN status IN ('IN_PROGRESS', 'FAILED', 'INSUFFICIENT') THEN status ELSE 'PENDING' END,
      updatedAt = NOW(3)
  `;
  const rows = await platformPrisma.$queryRaw<VerificationRow[]>`
    SELECT id, required, blocksEnforcement, status FROM ComplaintVerification
    WHERE tenantId = ${tenantId} AND complaintId = ${complaint.id} LIMIT 1
  `;
  return rows[0];
}

export async function recordComplaintVerification(user: EffectiveUser, input: { complaintId: string; status: ComplaintVerificationStatus; verificationType?: ComplaintVerificationType | null; findings?: string | null }) {
...
export async function assertComplaintEnforcementAllowed(tenantId: string, complaintId: string) {
  ...
  if (verification && Boolean(verification.required) && Boolean(verification.blocksEnforcement) && verification.status !== "PASSED") throw new Error("Independent verification is required before this enforcement action can proceed.");
  return verification;
}

export async function promoteComplaintToGrievance(user: EffectiveUser, complaintId: string) {
  await requireGrievancePermission(user, "TRIAGE_GRIEVANCE");
  const complaint = await platformPrisma.complaint.findFirst({ where: { tenantId: user.tenantId, id: complaintId }, select: { id: true, category: { select: { requiresBoardReview: true } } } });
  if (!complaint) throw new Error("Complaint not found.");
  const verification = await evaluateComplaintVerificationRequirement(user.tenantId, complaint.id);
  const status = verification && Boolean(verification.required) ? verification.status === "PASSED" ? "VERIFIED" : "VERIFICATION_REQUIRED" : "ASSESSMENT";
  const id = randomUUID();

  return platformPrisma.$transaction(async (tx) => {
    const affected = await tx.$executeRaw`
      INSERT IGNORE INTO GrievanceCase (id, tenantId, complaintId, status, boardReviewRequired, createdById, createdAt, updatedAt)
      VALUES (${id}, ${user.tenantId}, ${complaint.id}, ${status}, ${Boolean(complaint.category?.requiresBoardReview)}, ${user.id}, NOW(3), NOW(3))
    `;
    const rows = await tx.$queryRaw<GrievanceRow[]>`
      SELECT id, tenantId, complaintId, status, boardReviewRequired FROM GrievanceCase
      WHERE tenantId = ${user.tenantId} AND complaintId = ${complaint.id} LIMIT 1
    `;
    const grievance = rows[0];
    if (!grievance) throw new Error("Grievance case could not be created.");
    if (Number(affected) === 1) {
      const metadata = { status: grievance.status, boardReviewRequired: Boolean(grievance.boardReviewRequired) };
      await tx.$executeRaw`
        INSERT INTO ComplaintGrievanceActivity (id, tenantId, complaintId, grievanceCaseId, actorId, eventType, message, metadata, createdAt)
        VALUES (${randomUUID()}, ${user.tenantId}, ${complaint.id}, ${grievance.id}, ${user.id}, 'GRIEVANCE_CREATED', 'Complaint promoted to a formal grievance case.', ${JSON.stringify(metadata)}, NOW(3))
      `;
      await tx.auditLog.create({ data: { tenantId: user.tenantId, actorId: user.id, module: "COMPLAINTS", action: "PROMOTE_COMPLAINT_TO_GRIEVANCE", entityType: "GrievanceCase", entityId: grievance.id, metadata: { complaintId: complaint.id, ...metadata } } });
    }
    return grievance;
  });
}
...
export async function appointGrievanceCommitteeMember(user: EffectiveUser, input: { userId: string; position: GrievanceCommitteePosition; permissions: GrievancePermission[]; startsAt: Date; endsAt?: Date | null }) {
  ...
  const id = randomUUID();
  await platformPrisma.$executeRaw`
    INSERT INTO GrievanceCommitteeMembership (id, tenantId, userId, position, permissions, startsAt, endsAt, active, appointedById, createdAt, updatedAt)
    VALUES (${id}, ${user.tenantId}, ${member.id}, ${input.position}, ${JSON.stringify(permissions)}, ${input.startsAt}, ${input.endsAt ?? null}, true, ${user.id}, NOW(3), NOW(3))
  `;
...
export async function endGrievanceCommitteeMembership(user: EffectiveUser, membershipId: string, endedAt = new Date()) {
  ...
  const changed = await platformPrisma.$executeRaw`
    UPDATE GrievanceCommitteeMembership SET active = false, endsAt = ${endedAt}, updatedAt = NOW(3)
...
export async function createGrievanceDeadline(user: EffectiveUser, input: { grievanceCaseId: string; deadlineType: GrievanceDeadlineType; startsAt: Date; dueAt: Date; policySource?: string | null }) {
  await requireGrievancePermission(user, "TRIAGE_GRIEVANCE");
  ...
  const cases = await platformPrisma.$queryRaw<Array<{ id: string; complaintId: string }>>`
    SELECT id, complaintId FROM GrievanceCase WHERE tenantId = ${user.tenantId} AND id = ${input.grievanceCaseId} LIMIT 1
  `;
...
export async function pauseGrievanceOperationalSla(user: EffectiveUser, grievanceCaseId: string, reason: string) {
  ...
  const cases = await platformPrisma.$queryRaw<Array<{ id: string; complaintId: string }>>`
    SELECT id, complaintId FROM GrievanceCase WHERE tenantId = ${user.tenantId} AND id = ${grievanceCaseId} LIMIT 1
  `;
  const grievance = cases[0];
  if (!grievance) throw new Error("Grievance case not found.");
  await platformPrisma.$executeRaw`
    UPDATE GrievanceCase SET operationalSlaPausedAt = NOW(3), operationalSlaPauseReason = ${pauseReason}, updatedAt = NOW(3)
