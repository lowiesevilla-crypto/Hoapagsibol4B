...
export const settingSections: { category: SystemSettingCategory; title: string; description: string; fields: SettingField[] }[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { category: SystemSettingCategory.EMAIL, key: "MAIL_DKIM_SELECTOR", label: "DKIM selector(s)", help: "For Hostinger, enter hostingermail-a,hostingermail-b,hostingermail-c. HOAHub verifies each selector._domainkey sender DNS record before production email rollout.", placeholder: "hostingermail-a,hostingermail-b,hostingermail-c" },
      { category: SystemSettingCategory.EMAIL, key: "MAIL_DMARC_RECORD", label: "DMARC DNS TXT value", help: "Reference value for the sender domain DNS TXT record at _dmarc. Configure this in Hostinger DNS; HOAHub verifies the live DNS record.", placeholder: "v=DMARC1; p=none" },
