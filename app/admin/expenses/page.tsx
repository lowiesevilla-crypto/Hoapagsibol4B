...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ edit?: string }> }) {
  ...
  return <><PageHeader eyebrow="Financials" title="Expenses" description={`Track disbursements using administrator-defined categories. Total recorded: ${money(total)}.`} />
    ...
    <div className="table-wrap"><StandardTable><table className="data-table"><thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Payee</th><th>Voucher / reference</th><th>Source</th><th>Method</th><th className="text-right">Amount</th><th></th></tr></thead><tbody>{expenses.map((expense) => {
    ...
    })}{!expenses.length && <tr><td colSpan={9} className="py-12 text-center text-slate-500">No expenses recorded yet.</td></tr>}</tbody></table></StandardTable></div>
  </>;
}
