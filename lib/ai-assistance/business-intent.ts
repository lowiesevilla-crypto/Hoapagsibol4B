...
const ORGANIZATION_TERMS = /\b(president|vice president|secretary|treasurer|auditor|officer|officers|board|director|directors|committee|organization|organisation|head|heads|lead|leads|leadership|hoa leadership|association leadership)\b/i;
const RESIDENT_OWN_RECORD_TERMS = /\b(my|own|mine|ako|ko)\b.{0,90}\b(balance|billing|bill|dues|payment|payments|receipt|receipts|account|profile|property|block|lot|address|request|requests|complaint|complaints|collection|bond|refund|statement of account|soa)\b|\b(balance|billing|bills|dues|payment history|receipt|account number|statement of account|soa)\b/i;
const RESIDENT_TRANSACTION_TERMS = /\b(my|own|mine|ako|ko)\b.{0,90}\b(transaction|transactions|transaction history)\b|\b(transaction history|my transactions|recent transactions|latest transaction)\b/i;
...
export function classifyAiBusinessIntent(experience: Experience, question: unknown): AiBusinessIntent | null {
  ...
  // Drafting is an operational staff workflow even when the requested draft contains
  // words such as "board", "resolution", or "policy" that also appear in knowledge sources.
  if (DRAFT_RESOLUTION.test(value)) return "STAFF_EXISTING_OPERATIONAL";
  ...
  if (/\b(total collection today|today'?s collections?|finance summary|financial summary|cash flow|income|expenses?|net collection|what can you do|staff copilot|admin assistant|joke)\b/i.test(value)) return "STAFF_EXISTING_OPERATIONAL";
  return null;
}
