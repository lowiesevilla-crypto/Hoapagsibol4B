...
async function main() {
  ...
  const logoutTransitionScript = read("app/api/auth/logout-transition-script/route.ts");
  ...
  assert(!authButtons.includes("event.preventDefault()") && !authButtons.includes("useActionState") && !authButtons.includes("fetch("), "Logout must not depend on delegated submit suppression, React action state, or client fetch authority.");
  assert(!authButtons.includes("location.replace") && !authButtons.includes("logoutNavigationAction"), "Logout must not depend on client redirect authority or a React Server Action revocation path.");
  ...
  assert(logoutTransitionRoute.includes('src="/api/auth/logout-transition-script"') && logoutTransitionRoute.includes("defer"), "Logout transition must load its submitter from the dedicated same-origin no-store script route.");
  assert(logoutTransitionRoute.includes("privateNoStoreHeaders") && logoutTransitionRoute.includes("script-src 'self'") && logoutTransitionRoute.includes("form-action 'self'") && logoutTransitionRoute.includes("frame-ancestors 'none'"), "Logout transition document must be private/no-store and CSP-restricted to same-origin script and form actions.");
  assert(logoutTransitionScript.includes('form[data-hoahub-logout-transition="true"]') && logoutTransitionScript.includes("HTMLFormElement.prototype.submit.call(form)"), "The isolated transition script must perform the native POST outside React's event system.");
  assert(logoutTransitionScript.includes("privateNoStoreHeaders") && logoutTransitionScript.includes('"cross-origin-resource-policy": "same-origin"') && logoutTransitionScript.includes('"x-content-type-options": "nosniff"'), "Logout transition script must remain private/no-store and same-origin resource restricted.");
