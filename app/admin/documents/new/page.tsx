...
export default async function NewAdminDocumentPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const query = await searchParams; const [homeowners, officers, templates] = await Promise.all([prisma.homeownerProfile.findMany({ where: { tenantId: user.tenantId, status: "ACTIVE" }, include: { user: true }, orderBy: { user: { name: "asc" } } }), getActiveOrganizationOfficers(user.tenantId), prisma.documentTemplate.findMany({ where: { tenantId: user.tenantId, active: true }, orderBy: { title: "asc" } })]);
