...
export default async function PlatformPlansPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <>
    <PageHeader eyebrow="Platform revenue" title="Subscription plan catalog" description="Control pricing, commercial limits, trial duration, and the modules included in every HOAHub plan." action={<Link className="btn-secondary" href="/platform/tenants">Back to tenants</Link>} />
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {plans.map((plan) => <article key={plan.id} className="card flex min-h-64 flex-col">
        ...
        <form action={toggleSubscriptionPlanAction} className="mt-auto pt-5"><input type="hidden" name="planId" value={plan.id} /><button className="btn-secondary w-full">{plan.active ? "Deactivate plan" : "Activate plan"}</button></form>
      </article>)}
    ...
    <form action={createSubscriptionPlanAction} className="mt-6 rounded-3xl border bg-white p-5 shadow-sm sm:p-7">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <label><span className="label">Setup fee</span><input className="field" name="setupFee" type="number" min="0" step="0.01" defaultValue="0" /></label>
