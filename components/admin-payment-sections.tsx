...
export function RecordPaymentView({ data, query }: { data: Awaited<ReturnType<typeof getRecordPaymentData>>; query: PaymentQuery }) {
  return <>
    ...
    <PaginationFocusTarget id="record-payment-results" label="Record payment open bills" />
    ...
    <Pager page={data.billPage} count={data.openBillCount} pageSize={recordBillPageSize} pageKey="billPage" preserved={preservedEntries(query, ["billPage"])} basePath="/admin/payments/record" targetId="record-payment-results" />
  </>;
}
...
export function PaymentRequestsView({ data, query }: { data: Awaited<ReturnType<typeof getPaymentRequestsData>>; query: PaymentQuery }) {
  return <>
    ...
    <section className="card mb-6">
      <PaginationFocusTarget id="payment-requests-table" label="Payment requests table" />
    ...
    <Pager page={data.requestPage} count={data.requestCount} pageSize={paymentPageSize} pageKey="requestPage" preserved={preservedEntries(query, ["requestPage"])} basePath="/admin/payments/requests" targetId="payment-requests-table" />
  </>;
}
...
export function ActivePaymentsView({ data, query }: { data: Awaited<ReturnType<typeof getActivePaymentsData>>; query: PaymentQuery }) {
  return <>
    ...
    <section className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><PaginationFocusTarget id="active-payments-table" label="Active payments table" /><div><h2 className="text-lg font-black">Active payment records</h2><p className="text-sm text-slate-500">Controlled amount updates are audited. Use Void to remove a transaction from active financial totals.</p></div></section>
    <div className="table-wrap"><StandardTable><table className="data-table"><thead><tr><th>Receipt / reference</th><th>Homeowner</th><th>Payment Coverage</th><th>Payment date</th><th>Method</th><th>Proof</th><th>Controlled amount update</th><th>Actions</th></tr></thead><tbody>
    ...
    <Pager page={data.paymentPage} count={data.paymentCount} pageSize={paymentPageSize} pageKey="paymentPage" preserved={preservedEntries(query, ["paymentPage"])} basePath="/admin/payments/active" targetId="active-payments-table" />
  </>;
}
...
export function PaymentHistoryView({ data, query }: { data: Awaited<ReturnType<typeof getPaymentHistoryData>>; query: PaymentQuery }) {
  return <>
    ...
    <section className="card mt-6">
      <PaginationFocusTarget id="transaction-history-table" label="Transaction history table" />
      <div className="mb-4 flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-slate-100 text-slate-600"><History className="size-5" /></span><div><h2 className="text-lg font-black">Transaction history</h2><p className="text-sm text-slate-500">One row represents one Payment header. Voided receipts and allocations remain available for audit without affecting active balances.</p></div></div>
      <div className="table-wrap shadow-none"><StandardTable><table className="data-table"><thead><tr><th>Payment date</th><th>Official Receipt</th><th>Payer</th><th className="text-right">Received</th><th className="text-right">Applied</th><th className="text-right">Credit</th><th>Method / reference</th><th>Status</th><th>Processor</th><th>Allocations</th></tr></thead><tbody>
        {data.payments.map((payment) => { const processor = paymentProcessorIdentity(payment.processedBy); return <tr key={payment.id}><td>{shortDate(payment.paymentDate)}</td><td><Link className="font-mono text-xs font-black text-pine-700" href={`/receipts/payment/${payment.id}`} target="_blank">{payment.receiptNumber || "Legacy receipt"}</Link></td><td className="font-bold">{payment.homeowner.user.name}</td><td className="text-right font-black">{money(payment.amount)}</td><td className="text-right">{money(paymentAppliedAmount(payment))}</td><td className="text-right">{money(paymentUnappliedCredit(payment))}</td><td><p>{payment.method.replaceAll("_", " ")}</p><p className="font-mono text-xs text-slate-500">{payment.referenceNumber || "Not required"}</p></td><td><StatusBadge status={payment.status === "VOIDED" ? "VOID" : "ACTIVE"} />{payment.voidedAt && <p className="mt-1 text-xs text-slate-500">{shortDate(payment.voidedAt)}</p>}</td><td><p className="font-bold">{processor.name}</p><p className="text-xs text-slate-500">{processor.role}</p></td><td><details><summary className="cursor-pointer font-bold">{payment.allocations.length || 1} item(s)</summary><div className="mt-2 min-w-52 space-y-2">{payment.allocations.map((allocation) => <p key={allocation.id} className="text-xs"><b>{allocation.coverageLabel || monthLabel(allocation.bill.billingMonth)}</b><br />{allocation.bill.recurringChargeType.replaceAll("_", " ")} - {money(allocation.amount)}</p>)}{!payment.allocations.length && <p className="text-xs">{paymentCoverageLabel(payment)} - {money(payment.amount)}</p>}{payment.status === "VOIDED" && <p className="text-xs text-rose-700">{payment.voidReason || "Voided payment"}</p>}</div></details></td></tr>; })}
        {!data.payments.length && <tr><td colSpan={10} className="py-10 text-center text-slate-500">No transaction history found.</td></tr>}
      </tbody></table></StandardTable></div>
    </section>
    <Pager page={data.historyPage} count={data.paymentCount} pageSize={paymentPageSize} pageKey="historyPage" preserved={preservedEntries(query, ["historyPage"])} basePath="/admin/payments/history" targetId="transaction-history-table" />
  </>;
}

function paymentRequestPurpose(request: { type: PaymentRequestType; bill?: { billingMonth: Date } | null; collectionType?: unknown; description?: string | null; documentRequest?: { definition?: { displayName: string } | null; configuration?: { displayName: string } | null; type?: unknown } | null }) {
  if (request.type === PaymentRequestType.MONTHLY_DUES) return `Monthly dues - ${request.bill ? request.bill.billingMonth.toLocaleDateString("en-PH", { month: "long", year: "numeric" }) : "Bill"}`;
  if (request.type === PaymentRequestType.DOCUMENT_FEE) return `Document Request Fee - ${request.documentRequest?.definition?.displayName || request.documentRequest?.configuration?.displayName || (request.documentRequest?.type ? documentTypeLabel(request.documentRequest.type as never) : "Official HOA document")}`;
...
function PaymentFilterPanel({ action, query, homeowners, includeRequestFilters = false, includePaymentFilters = false }: { action: string; query: PaymentQuery; homeowners: HomeownerFilter[]; includeRequestFilters?: boolean; includePaymentFilters?: boolean }) {
  return <form action={action} className="card mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
    ...
    {includeRequestFilters && <select className="field" name="paymentType" defaultValue={query.paymentType || ""}><option value="">All payment types</option><option value="MONTHLY_DUES">Monthly dues</option><option value="OTHER_COLLECTION">Other collection</option><option value="DOCUMENT_FEE">Document fee</option></select>}
...
function Pager({ page, count, pageSize, pageKey, preserved, basePath, targetId }: { page: number; count: number; pageSize: number; pageKey: string; preserved: string[][]; basePath: string; targetId: string }) {
  const pages = Math.max(1, Math.ceil(count / pageSize));
  if (pages <= 1) return null;
  const url = (target: number) => {
    const params = new URLSearchParams(preserved);
    params.set(pageKey, String(target));
    return `${basePath}?${params}#${targetId}`;
  };
