...
import { PlatformManualPaymentAvailabilityPanel } from "@/components/platform-manual-payment-availability-panel";
...
export function PlatformTenantTabs({ tenantId, active }: { tenantId: string; active: string }) {
  const tabs = [
    ...
    { key: "lifecycle", label: "Lifecycle", href: `/platform/tenants/${tenantId}/lifecycle` },
  ];
  return <>
    ...
    {active === "billing" && <PlatformManualPaymentAvailabilityPanel tenantId={tenantId} />}
