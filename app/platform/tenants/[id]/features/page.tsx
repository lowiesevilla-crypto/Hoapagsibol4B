...
function overrideMode(value: boolean | null | undefined) {
  return value === true ? "ENABLE" : value === false ? "DISABLE" : "INHERIT";
}
...
export default async function PlatformTenantFeaturesPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const documentEnabled = Boolean((documentOverride?.enabledOverride ?? documentPlan?.enabled ?? false) && commerciallyActive);
  const aiEnabled = Boolean((aiOverride?.enabledOverride ?? aiPlan?.enabled ?? false) && commerciallyActive);
  ...
  return <div>
    ...
    <section className="mt-5 grid gap-4 md:grid-cols-3">
      <article className="card"><p className="text-xs font-black uppercase text-slate-400">Subscribed plan</p><p className="mt-2 text-xl font-black">{plan?.name ?? tenant.subscriptionPlan}</p><p className="mt-1 text-sm text-slate-500">{subscriptionStatus.replaceAll("_", " ")}</p></article>
      <article className="card"><p className="text-xs font-black uppercase text-slate-400">Document Management</p><p className={`mt-2 text-xl font-black ${documentEnabled ? "text-emerald-700" : "text-slate-500"}`}>{documentEnabled ? "Enabled" : "Disabled"}</p><p className="mt-1 text-sm text-slate-500">{documentOverride?.enabledOverride != null ? "Tenant override" : "Plan inheritance"}</p></article>
      <article className="card"><p className="text-xs font-black uppercase text-slate-400">AI Assistance</p><p className={`mt-2 text-xl font-black ${aiEnabled ? "text-emerald-700" : "text-slate-500"}`}>{aiEnabled ? "Commercially enabled" : "Disabled"}</p><p className="mt-1 text-sm text-slate-500">Runtime still requires tenant AI governance approval.</p></article>
    </section>
    ...
    <form action={updateTenantFeatureEntitlementsAction} className="mt-6 space-y-6">
      ...
      <section className="rounded-3xl border bg-white p-5 shadow-sm sm:p-7">
        <div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-pine-50 text-pine-700"><FolderLock className="size-5" /></span><div><p className="text-xs font-black uppercase tracking-wider text-pine-700">Repository capability</p><h2 className="text-xl font-black">Document Management</h2><p className="mt-1 text-sm text-slate-500">Plan: {documentPlan?.enabled ? "included" : "not included"}. Blank limit fields inherit the plan.</p></div></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <label><span className="label">Entitlement</span><select className="field" name="documentEnabledOverride" defaultValue={overrideMode(documentOverride?.enabledOverride)}><option value="INHERIT">Inherit plan</option><option value="ENABLE">Force enable</option><option value="DISABLE">Force disable</option></select></label>
          ...
          <label><span className="label">Revision file retention</span><select className="field" name="documentRetainRevisionBinariesOverride" defaultValue={overrideMode(documentOverride?.retainRevisionBinariesOverride)}><option value="INHERIT">Inherit plan</option><option value="ENABLE">Force retain</option><option value="DISABLE">Force metadata only</option></select></label>
        ...
        <p className="mt-4 rounded-2xl bg-blue-50 p-4 text-xs leading-5 text-blue-800">Disabling or reducing quota never automatically deletes existing repository files. Over-quota tenants retain authorized read/download access while new managed storage is blocked.</p>
      </section>

      <section className="rounded-3xl border border-indigo-100 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-indigo-50 text-indigo-700"><Bot className="size-5" /></span><div><p className="text-xs font-black uppercase tracking-wider text-indigo-700">Sellable AI capability</p><h2 className="text-xl font-black">HOAHub AI Assistance</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">Commercial entitlement is independent of Document Management. Enabling it here does not bypass tenant AI governance, privacy, DPO/provider review, role policy, or runtime kill switches.</p></div></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <label><span className="label">Entitlement</span><select className="field" name="aiEnabledOverride" defaultValue={overrideMode(aiOverride?.enabledOverride)}><option value="INHERIT">Inherit plan</option><option value="ENABLE">Force enable</option><option value="DISABLE">Force disable</option></select></label>
        ...
        <div className="mt-5 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><ShieldCheck className="mt-0.5 size-5 shrink-0" /><p><b>Privacy gate:</b> Production AI must remain unavailable until the tenant has an approved lawful basis/PIA, privacy notice, provider and cross-border review, role/audience policy, retention settings, and tenant-isolation UAT.</p></div>
      </section>

      <button className="btn-primary min-h-12 w-full sm:w-auto">Save tenant feature controls</button>
    </form>
  </div>;
}
