  ...
  return <>
    ...
    {section === "settings" && <section className="space-y-6">
      {canManagePayroll && <section className="card">
        ...
        <StandardTable><div className="table-wrap shadow-none"><table className="data-table"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Granted by</th><th></th></tr></thead><tbody>{payrollAccesses.map((item) => <tr key={item.id}><td><p className="font-bold">{item.user.name}</p><p className="text-xs text-slate-400">{item.user.email}</p></td><td>{payrollRoleLabel(item.role)}</td><td><StatusBadge status={item.active ? "ACTIVE" : "INACTIVE"} /></td><td>{item.grantedBy?.name ?? "-"}</td><td><form action={deletePayrollAccessAction}><input type="hidden" name="id" value={item.id} /><DeleteButton label="Remove" /></form></td></tr>)}{!payrollAccesses.length && <tr><td colSpan={5} className="py-10 text-center text-slate-500">No payroll access assignments yet.</td></tr>}</tbody></table></div></StandardTable>
      </section>}
