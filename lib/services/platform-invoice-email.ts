...
import { maskEmailAddress } from "@/lib/services/email-delivery-safety";
import { sendProtectedRawEmail } from "@/lib/services/notifications";
  ...
  await prisma.auditLog.create({
    data: {
      ...
      metadata: {
        ...
        maskedRecipients: input.recipients,
        providerMessageIdPresent: Boolean(input.providerMessageId),
...
export async function sendPlatformInvoiceEmail(input: { invoiceId: string; actorId?: string }): Promise<PlatformInvoiceEmailResult> {
  ...
  for (const recipient of recipientAddresses) {
    ...
    if (delivery.status === "SENT") {
      ...
      if (delivery.failureKind === "PROVIDER_CIRCUIT") break;
    } else {
      failed++;
      lastMessage = delivery.message;
      if (delivery.failureKind === "PROVIDER_CIRCUIT") break;
  ...
  const status: PlatformInvoiceEmailResult["status"] = failed > 0 ? "FAILED" : sent > 0 ? "SENT" : "SKIPPED";
  const result: PlatformInvoiceEmailResult = {
    status,
    recipients,
    providerMessageId,
    message: failed > 0
      ? `Platform invoice delivery stopped safely. Sent: ${sent}; skipped: ${skipped}; failed: ${failed}. ${lastMessage || ""}`.trim()
      : skipped > 0
        ? `Platform invoice delivery completed with protected skips. Sent: ${sent}; skipped: ${skipped}. ${lastMessage || ""}`.trim()
        : undefined,
  };
  await auditDelivery({
    tenantId: invoice.tenantId,
