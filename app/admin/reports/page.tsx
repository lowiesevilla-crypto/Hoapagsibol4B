...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <>
    ...
    <section className="mt-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2"><Statement title="Statement of Income and Expenses" subtitle={`${shortDate(from)} to ${shortDate(to)}`}><SectionLabel text="REVENUE" /><Row label="Homeowner monthly dues" value={duesIncome} />{[...feeBreakdown.entries()].map(([label, value]) => <Row key={label} label={value} />)}<Row label="Forfeited bond income" value={forfeitedIncome} /><Row label="Total revenue" value={recognizedIncome} total /><SectionLabel text="OPERATING EXPENSES" />{[...expenseBreakdown.entries()].map(([label, value]) => <Row key={label} label={label} value={value} />)}<Row label="Employee payroll" value={payrollExpense} /><Row label="Total operating expenses" value={totalExpenses} total /><Row label="NET OPERATING SURPLUS / (DEFICIT)" value={operatingSurplus} grand /></Statement>
