  ...
  return <form action={recordHomeownerPaymentAction} className="card mb-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.15fr_1fr]">
      ...
      <div className="grid content-start gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2"><label className="label">Payment amount received <span className="text-rose-600">*</span></label><input className="field text-right text-lg font-black text-pine-700" name="amount" type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} required placeholder="0.00" /><p className="mt-1 text-xs text-slate-500">Any amount not applied to an existing bill remains available as homeowner credit.</p></div>
        ...
        <div className="sm:col-span-2"><label className="label">Reference number {referenceRequired && <span className="text-rose-600">*</span>}</label><input className="field" name="referenceNumber" required={referenceRequired} aria-required={referenceRequired} placeholder={referenceRequired ? "Required; must be unique" : "Optional for cash payments"} /></div>
