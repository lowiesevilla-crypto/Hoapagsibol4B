...
import { CERTIFICATE_OF_RESIDENCY_CERTIFIED_KEY, CERTIFICATE_OF_RESIDENCY_CODE, CERTIFICATE_OF_RESIDENCY_REFERENCE_TEMPLATE_NAME } from "@/lib/services/certificate-of-residency";
import { cloneCertifiedCertificateTemplateAction, createCertificateReferenceDraftAction, restoreCertifiedCertificateTemplateAction } from "@/lib/actions/certificate-of-residency";
...
export default async function DefinitionTemplatesPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string; message?: string }> }) {
  ...
  const referenceSet = definition.templateSets.find((set) => set.name === CERTIFICATE_OF_RESIDENCY_REFERENCE_TEMPLATE_NAME) ?? null;
  const ownership = currentSet ? getTemplateOwnershipMetadata(currentSet, currentSet.versions) : null;
  const certified = definition.code === CERTIFICATE_OF_RESIDENCY_CODE ? await platformPrisma.documentTemplateVersion.findFirst({ where: { tenantId: CERTIFIED_TEMPLATE_TENANT_ID, status: DocumentTemplateVersionStatus.PUBLISHED, ownershipType: DocumentTemplateOwnership.CERTIFIED, templateSet: { certifiedKey: CERTIFICATE_OF_RESIDENCY_CERTIFIED_KEY, active: true } }, include: { templateSet: true }, orderBy: { version: "desc" } }) : null;
  return <>
    <PageHeader eyebrow="Document templates" title={definition.displayName} description="Manage the draft, published version, and immutable version history for this document type." />
    {query.error && <Notice kind="error">{query.error}</Notice>}
    {query.success && <Notice kind="success">{query.message || "Template updated."}</Notice>}
    {definition.code === CERTIFICATE_OF_RESIDENCY_CODE && <section className="card mb-5"><h2 className="text-lg font-black">HOAHub Certified Template</h2><p className="mt-1 text-sm text-slate-500">The certified source is platform-owned and read-only. Tenant changes are made only in an independent draft.</p>{certified ? <div className="mt-4 grid gap-4 md:grid-cols-[1fr_auto] md:items-end"><div className="grid gap-3 text-sm sm:grid-cols-3"><Info label="Certified template" value={certified.templateSet.name} /><Info label="Published version" value={`v${certified.version}`} /><Info label="Published date" value={certified.publishedAt ? shortDate(certified.publishedAt) : "Not recorded"} /></div><div className="flex flex-wrap gap-2">{!definition.templateSets.some((set) => set.sourceTemplateSetId === certified.templateSetId) && <form action={cloneCertifiedCertificateTemplateAction}><input type="hidden" name="definitionId" value={definition.id} /><input type="hidden" name="certifiedVersionId" value={certified.id} /><button className="btn-primary">Clone certified template</button></form>}{!referenceSet && <form action={createCertificateReferenceDraftAction}><input type="hidden" name="definitionId" value={definition.id} /><input type="hidden" name="certifiedVersionId" value={certified.id} /><button className="btn-secondary">Create visual reference draft</button></form>}{currentSet && currentSet.ownershipType !== DocumentTemplateOwnership.CERTIFIED && <form action={restoreCertifiedCertificateTemplateAction}><input type="hidden" name="definitionId" value={definition.id} /><input type="hidden" name="templateSetId" value={currentSet.id} /><input type="hidden" name="certifiedVersionId" value={certified.id} /><button className="btn-secondary">Restore default as draft</button></form>}</div></div> : <p className="mt-4 rounded-xl bg-amber-50 p-4 text-sm font-bold text-amber-900">The controlled HOAHub certified template seed has not been run in this environment.</p>}</section>}
