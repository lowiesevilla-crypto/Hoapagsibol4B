...
export default async function PortalCollectionsPage() {
  ...
  const collections = await prisma.collection.findMany({ where: { homeownerId: profile.id }, include: { refunds: { orderBy: { refundDate: "desc" } } }, orderBy: [{ collectionDate: "desc" }, { createdAt: "desc" }] });
  return <><PageHeader eyebrow="My account" title="Other collections & bonds" description="Your gate passes, stickers, memberships, other fees, and refundable construction bonds." /><StandardTable><div className="table-wrap"><table className="data-table"><thead><tr><th>Type / remarks</th><th>Collection date</th><th>Amount</th><th>Refunded</th><th>Balance held</th><th>Status</th><th>Receipt / reference</th><th></th></tr></thead><tbody>{collections.map((item) => {
  ...
  })}{!collections.length && <tr><td colSpan={8} className="py-12 text-center text-slate-500">No other collections are associated with your account.</td></tr>}</tbody></table></div></StandardTable></>;
}
