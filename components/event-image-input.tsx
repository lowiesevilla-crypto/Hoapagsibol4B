...
export function EventImageInput({ currentImage, title }: { currentImage?: string | null; title: string }) {
  ...
  function change(file?: File) {
    ...
    if (![".jpg", ".jpeg", ".png"].includes(extension) || !allowedTypes.includes(file.type)) { setPreview(currentImage ?? ""); setError("Choose a JPG, JPEG, or PNG image."); return; }
  ...
  return <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-4">
    ...
    <input className="field bg-white" name="image" type="file" accept=".jpg,.jpeg,.png" onChange={(event) => change(event.target.files?.[0])} />
