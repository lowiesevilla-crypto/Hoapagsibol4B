...
export default async function PlatformInvoicePaymentPage({ params, searchParams }: { params: Promise<{ invoiceId: string }>; searchParams: Promise<{ token?: string; result?: string; error?: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8 sm:py-14"><div className="mx-auto max-w-2xl">
    <div className="rounded-3xl bg-gradient-to-r from-pine-950 to-pine-700 p-6 text-white shadow-xl sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-leaf-100">HOAHub secure billing</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">{invoice.tenant.billingProfile?.legalBusinessName || invoice.tenant.name}</h1><p className="mt-2 text-sm text-pine-50">Subscription invoice {invoice.invoiceNumber}</p></div><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-white/15"><ShieldCheck className="size-6" /></span></div></div>
    ...
    <section className="mt-5 rounded-3xl border bg-white p-5 shadow-sm sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-slate-500">Amount due</p><p className="mt-1 text-4xl font-black text-pine-950">{money(Number(invoice.outstandingBalance), invoice.currency)}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${paid ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-700"}`}>{invoice.status.replaceAll("_", " ")}</span></div>
      ...
      <div className="mt-5 space-y-2 text-sm"><div className="flex justify-between"><span className="text-slate-500">Invoice total</span><span className="font-bold">{money(Number(invoice.total), invoice.currency)}</span></div><div className="flex justify-between"><span className="text-slate-500">Amount paid</span><span className="font-bold">{money(Number(invoice.amountPaid), invoice.currency)}</span></div><div className="flex justify-between border-t pt-3 text-base"><span className="font-black">Outstanding</span><span className="font-black text-pine-950">{money(Number(invoice.outstandingBalance), invoice.currency)}</span></div></div>
      {paid ? <div className="mt-6 flex items-start gap-3 rounded-2xl bg-emerald-50 p-4 text-emerald-900"><CheckCircle2 className="mt-0.5 size-5 shrink-0" /><div><p className="font-black">Payment confirmed</p><p className="mt-1 text-sm">This HOAHub subscription invoice is fully paid.</p></div></div> : payMongoIsConfigured() ? <form action={startPayMongoInvoiceCheckoutAction} className="mt-6"><input type="hidden" name="invoiceId" value={invoice.id} /><input type="hidden" name="token" value={token} /><button className="btn-primary flex min-h-14 w-full items-center justify-center gap-2 text-base"><CreditCard className="size-5" />Pay {money(Number(invoice.outstandingBalance), invoice.currency)} online</button><p className="mt-3 text-center text-xs leading-5 text-slate-500">You will complete payment on PayMongo's hosted checkout. HOAHub posts payment only after a verified gateway webhook.</p></form> : <div className="mt-6 rounded-2xl bg-amber-50 p-4 text-sm text-amber-900"><p className="font-black">Online payment setup pending</p><p className="mt-1">Please contact HOAHub billing support for payment instructions.</p></div>}
    </section>
