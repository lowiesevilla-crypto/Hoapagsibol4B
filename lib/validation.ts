...
export const emailSettingsSchema = z.object({
  ...
  MAIL_DKIM_SELECTOR: z.string().trim().regex(/^[A-Za-z0-9_, -]*$/, "DKIM selectors may contain letters, numbers, underscores, hyphens, commas, or spaces only.").max(300).optional(),
  MAIL_DMARC_RECORD: z.string().trim().regex(/^$|^v=DMARC1\b/i, "DMARC record must start with v=DMARC1.").max(500).optional(),
