...
export async function submitPaymentRequestAction(formData: FormData) {
  ...
  try {
    ...
    const duplicatePayment = await prisma.payment.findFirst({ where: { referenceNumber } });
    if (duplicatePayment) throw new Error("This payment reference number has already been recorded.");
    const duplicateRequest = await prisma.paymentRequest.findFirst({ where: { referenceNumber, status: { not: "REJECTED" } } });
    ...
    const bills = await prisma.bill.findMany({
      where: { id: { in: uniqueBillIds }, homeownerId: user.homeownerProfile.id, balance: { gt: 0 }, archivedAt: null },
...
export async function rejectPaymentRequestAction(formData: FormData) {
  ...
  const parsed = paymentReviewSchema.safeParse(Object.fromEntries(formData.entries()));
  if (!parsed.success) throw new Error(parsed.error.issues[0]?.message || "Invalid review details.");
  await rejectPaymentRequest(parsed.data.id, admin.id, parsed.data.reviewRemarks);
  revalidatePaymentPages();
  redirect("/admin/payments?success=rejected&message=QR%20payment%20request%20has%20been%20rejected.");
}
