...
export const ONBOARDING_HOMEOWNER_BATCH_SIZE = 500;
...
export function parseOnboardingHomeownerCsv(input: string): ParsedOnboardingCsv {
  ...
  if (matrix.length > ONBOARDING_HOMEOWNER_BATCH_SIZE) errors.push({ rowNumber: null, field: null, message: `Each onboarding batch is limited to ${ONBOARDING_HOMEOWNER_BATCH_SIZE} homeowner rows. Split larger communities into multiple batches; previously applied batches are preserved.` });
