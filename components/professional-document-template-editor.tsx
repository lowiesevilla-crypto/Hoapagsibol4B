          ...
          <div className="overflow-auto rounded-xl border border-slate-300 bg-slate-200 p-8 shadow-inner print:overflow-visible print:border-0 print:bg-white print:p-0 print:shadow-none">
            <div className="relative mx-auto" style={{ width: paperWidth * scale, minHeight: paperHeight * scale }}>
              ...
              >
                ...
                {definition.page.backgroundImage && <div className="pointer-events-none absolute inset-0 z-0" style={{ backgroundImage: `url(${definition.page.backgroundImage.src})`, backgroundSize: definition.page.backgroundImage.fit === "fill" ? "100% 100%" : definition.page.backgroundImage.fit, backgroundPosition: definition.page.backgroundImage.position, backgroundRepeat: "no-repeat", opacity: definition.page.backgroundImage.opacity }} />}
...
function CanvasBlock({ block, selected, editing, editable, scale, onSelect, onDoubleClick, onChangeText, onCommitText, onEscape, onPointerDown, onResizeStart, onImageUpload }: { block: VisualBlock; selected: boolean; editing: boolean; editable: boolean; scale: number; onSelect: (event: React.MouseEvent) => void; onDoubleClick: () => void; onChangeText: (content: string) => void; onCommitText: (content: string) => void; onEscape: () => void; onPointerDown: (event: React.PointerEvent, block: VisualBlock) => void; onResizeStart: (event: React.PointerEvent, block: VisualBlock, mode: "resize", corner: "nw" | "ne" | "sw" | "se") => void; onImageUpload: () => void }) {
  ...
  const content = editing ? <textarea autoFocus className="h-full w-full resize-none border-0 bg-transparent p-0 text-inherit outline-none" value={draft} onChange={(event) => { const value = sanitizeText(event.target.value); setDraft(value); onChangeText(value); }} onBlur={() => onCommitText(draft)} onKeyDown={(event) => { if (event.key === "Escape") { setDraft(block.content || ""); onCommitText(block.content || ""); onEscape(); } }} /> : <BlockContent block={block} onImageUpload={onImageUpload} />;
