...
export default async function EmployeeAttendanceHistoryPage() {
  ...
  return <>
    ...
    <section className="card mb-6"><h2 className="text-lg font-black">Attendance records</h2><div className="mt-4 table-wrap shadow-none"><StandardTable><table className="data-table"><thead><tr><th>Date</th><th>Status</th><th>Time</th><th>Total</th><th>Late / undertime</th><th>Remarks</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td>{shortDate(record.date)}</td><td><StatusBadge status={record.status} /></td><td>{record.timeIn || "-"} - {record.timeOut || "-"}</td><td>{String(record.totalHours)} hrs</td><td>{record.lateMinutes}m / {record.undertimeMinutes}m</td><td>{record.remarks || "-"}</td></tr>)}</tbody></table></StandardTable></div></section>
    <section className="card"><h2 className="text-lg font-black">Correction request history</h2><div className="mt-4 table-wrap shadow-none"><StandardTable><table className="data-table"><thead><tr><th>Requested</th><th>Attendance date</th><th>Reason</th><th>Status</th><th>Reviewed</th></tr></thead><tbody>{requests.map((item) => <tr key={item.id}><td>{shortDate(item.createdAt)}</td><td>{shortDate((item.originalData as Record<string, string>).date || item.createdAt)}</td><td>{item.reason}</td><td><StatusBadge status={item.status} /></td><td>{item.reviewedAt ? shortDate(item.reviewedAt) : "Pending"}</td></tr>)}{!requests.length && <tr><td colSpan={5} className="py-10 text-center text-slate-500">No correction requests yet.</td></tr>}</tbody></table></StandardTable></div></section>
  </>;
}
