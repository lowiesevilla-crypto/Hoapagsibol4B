...
async function main() {
  ...
  assert(!authButtons.includes("document.createElement(\"form\")") && !authButtons.includes("HTMLFormElement.prototype.submit.call"), "Protected React pages must not submit the logout POST directly.");
  ...
  assert(logoutTransitionRoute.includes('fetch("/api/auth/logout"') && logoutTransitionRoute.includes('method: "POST"') && logoutTransitionRoute.includes('credentials: "same-origin"') && logoutTransitionRoute.includes('redirect: "follow"') && logoutTransitionRoute.includes('cache: "no-store"'), "The isolated transition document must create a fresh same-origin POST outside the protected React tree.");
  ...
  assert(logoutTransitionRoute.includes("response.redirected") && logoutTransitionRoute.includes("new URL(response.url)") && logoutTransitionRoute.includes("destination.origin === window.location.origin") && logoutTransitionRoute.includes('destination.pathname === "/login"') && logoutTransitionRoute.includes('destination.pathname.endsWith("/login")') && logoutTransitionRoute.includes("window.location.replace(destination.href)"), "Transition navigation must use only the same-origin login destination resolved by the server POST redirect.");
  assert(logoutTransitionRoute.includes('data-hoahub-logout-retry="true"') && !logoutTransitionRoute.includes("HTMLFormElement.prototype.submit.call") && !logoutTransitionRoute.includes("window.setTimeout(submitLogout"), "Transition retries must be explicit and must not duplicate native POST submissions automatically.");
  ...
  assert(logoutRoute.includes("NextResponse.redirect(destination, 303)"), "Logout POST must finish with an HTTP 303 redirect after revocation.");
