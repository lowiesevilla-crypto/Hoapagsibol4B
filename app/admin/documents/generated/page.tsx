...
export default async function GeneratedDocumentsPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  return <>
    <PageHeader eyebrow="Document management" title="Generated documents" description="Latest active versions available to administrators and their respective homeowners." />
    ...
    <section className="card p-0 sm:p-0"><div className="table-wrap rounded-none shadow-none"><StandardTable><table className="data-table min-w-[850px]"><thead><tr><th>Document no.</th><th>Homeowner</th><th>Type</th><th>Origin</th><th>Version</th><th>Generated</th><th>Actions</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td className="font-mono font-bold">{item.documentNumber}</td><td>{item.homeowner.user.name}</td><td>{item.definition?.displayName || item.configuration?.displayName || documentTypeLabel(item.type)}</td><td>{item.origin === "ADMIN" ? "Admin / walk-in" : "Homeowner request"}</td><td>v{item.currentVersion}</td><td>{shortDate(item.generatedAt!)}</td><td><div className="flex gap-2"><Link className="btn-secondary min-h-9 px-3 py-1 text-xs" href={`/admin/documents/${item.id}`}>Manage</Link><a className="btn-primary min-h-9 px-3 py-1 text-xs" href={`/documents/${item.id}/pdf`}>PDF</a></div></td></tr>)}{!items.length && <tr><td colSpan={7} className="py-12 text-center text-slate-500">No generated documents found.</td></tr>}</tbody></table></StandardTable></div></section>
