...
export function PayMongoHomeownerForm({ openBills, documentPayment }: { openBills: OpenBill[]; documentPayment?: DocumentFeePayment | null }) {
  ...
  const displayAmount = documentPayment?.amountLabel || selectedBill?.balanceLabel || new Intl.NumberFormat("en-PH", { style: "currency", currency: "PHP" }).format(Number(amount) || 0);
  ...
  return <form action={createHomeownerPayMongoCheckoutAction} className="rounded-3xl border border-blue-100 bg-white shadow-soft">
    <div className="p-4 sm:p-5">
      <div className="mb-5 flex items-start gap-3">
        ...
        <div><h2 className="text-lg font-black">Pay securely online</h2><p className="text-sm leading-6 text-slate-500">Choose what to pay, then continue to PayMongo. HOAHub records the payment only after PayMongo sends a verified payment confirmation.</p></div>
      </div>
    ...
    <div className="sticky bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-20 border-t border-blue-100 bg-white/95 p-3 backdrop-blur lg:bottom-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-slate-500">Checkout amount</p><p className="text-xl font-black tabular-nums text-slate-950">{displayAmount}</p></div><PayMongoSubmitButton disabled={!canSubmit} /></div>
    </div>
  </form>;
}
