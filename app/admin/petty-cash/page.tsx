...
export default async function PettyCashPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <>
    <PageHeader
      ...
      description="Create traceable petty cash disbursements and manage each posted transaction from its Voucher View. Linked expense-history rows are read-only."
    ...
    {query.success === "deleted" && <div className="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">Voucher deleted successfully. Its linked expense entries were removed in the same transaction.</div>}
    {query.error && <div className="mb-5 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-900">{query.error}</div>}
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-2 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-black text-ink">Voucher register</h2><p className="mt-1 text-sm text-slate-500">Open a voucher to review, edit, delete or print it. Maintenance actions are intentionally kept inside Voucher View.</p></div><Link className="text-sm font-black text-pine-700 hover:underline" href="/admin/expenses">View expense history</Link></div>
      {vouchers.length ? <>
        ...
        <div className="hidden overflow-x-auto sm:block"><StandardTable><table className="w-full min-w-[760px] text-sm"><thead className="bg-slate-50 text-left text-xs font-black uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Voucher no.</th><th className="px-5 py-3">Date</th><th className="px-5 py-3">Payee</th><th className="px-5 py-3">Type</th><th className="px-5 py-3 text-center">Items</th><th className="px-5 py-3 text-right">Amount</th><th className="px-5 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{vouchers.map((voucher) => <tr key={voucher.id} className="hover:bg-slate-50"><td className="px-5 py-4 font-mono font-black text-pine-800">{voucher.voucherNumber}</td><td className="px-5 py-4">{shortDate(voucher.transactionDate)}</td><td className="px-5 py-4 font-bold text-ink">{voucher.payeeName}</td><td className="px-5 py-4 text-slate-500">{voucher.payeeType.replaceAll("_", " ")}</td><td className="px-5 py-4 text-center">{Number(voucher.itemCount)}</td><td className="px-5 py-4 text-right font-black">{money(Number(voucher.totalAmount))}</td><td className="px-5 py-4 text-right"><Link className="font-black text-pine-700 hover:underline" href={`/admin/petty-cash/${voucher.id}`}>Open Voucher</Link></td></tr>)}</tbody></table></StandardTable></div>
