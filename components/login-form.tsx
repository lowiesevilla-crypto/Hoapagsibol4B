...
export function LoginForm({ tenantSlug, returnTo }: { tenantSlug?: string; returnTo?: string }) {
  ...
  return (
    <form ref={formRef} action={action} className="community-pulse-form space-y-3 sm:space-y-4">
      ...
      <button
        ...
        aria-live="polite"
      >
        {verified ? <span className="inline-flex items-center justify-center gap-2"><CheckCircle2 className="size-5" /> Access verified</span> : pending ? "Signing in..." : hasChoices ? "Open selected account" : "Sign in securely"}
      </button>
