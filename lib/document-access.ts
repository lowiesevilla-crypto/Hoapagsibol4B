...
export async function getAccessibleGeneratedDocument(id: string, options?: { requireDownload?: boolean }) {
  ...
  const request = await prisma.documentRequest.findFirst({ where: { id, tenantId: user.tenantId }, include: { homeowner: { include: { user: true } }, definition: true, configuration: true, paymentRequest: true, processedBy: true, approvedBy: true, processedByOfficer: true, approvedByOfficer: true, versions: { orderBy: { version: "desc" }, take: 1 }, histories: { include: { actor: true }, orderBy: { createdAt: "asc" } } } });
