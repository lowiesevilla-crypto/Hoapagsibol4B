...
async function main() {
  ...
  assert(logoutTransitionRoute.includes('fetch("/api/auth/logout"') && logoutTransitionRoute.includes('method: "DELETE"') && logoutTransitionRoute.includes('credentials: "same-origin"') && logoutTransitionRoute.includes('redirect: "follow"') && logoutTransitionRoute.includes('cache: "no-store"'), "The isolated transition document must create a fresh same-origin non-POST mutation outside the protected React tree so stale Next-Action metadata cannot trigger Server Action dispatch.");
  assert(logoutTransitionRoute.includes('"Content-Type": "application/x-www-form-urlencoded;charset=UTF-8"') && logoutTransitionRoute.includes("new URLSearchParams({ scope }).toString()"), "Logout transition must submit only the bounded logout scope as URL-encoded form data.");
  assert(logoutTransitionRoute.includes("response.redirected") && logoutTransitionRoute.includes("new URL(response.url)") && logoutTransitionRoute.includes("destination.origin === window.location.origin") && logoutTransitionRoute.includes('destination.pathname === "/login"') && logoutTransitionRoute.includes('destination.pathname.endsWith("/login")') && logoutTransitionRoute.includes("window.location.replace(destination.href)"), "Transition navigation must use only the same-origin login destination resolved by the server redirect.");
  assert(logoutTransitionRoute.includes('data-hoahub-logout-retry="true"') && !logoutTransitionRoute.includes("HTMLFormElement.prototype.submit.call") && !logoutTransitionRoute.includes("window.setTimeout(submitLogout"), "Transition retries must be explicit and must not duplicate logout mutations automatically.");
  ...
  assert(logoutRoute.includes("export const POST = handleLogout") && logoutRoute.includes("export const DELETE = handleLogout"), "Logout route must keep POST compatibility while exposing the stale-Server-Action-safe DELETE mutation used by the isolated transition.");
  assert(logoutRoute.includes("NextResponse.redirect(destination, 303)"), "Logout mutations must finish with an HTTP 303 redirect after revocation.");
  assert(!logoutRoute.includes("X-HOA-Logout-Navigation") && !logoutRoute.includes("NextResponse.json("), "Logout route must keep one standard server redirect contract rather than a client-specific JSON branch.");
