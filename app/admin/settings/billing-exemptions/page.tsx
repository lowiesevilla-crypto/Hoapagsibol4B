...
export default async function BillingExemptionsPage({ searchParams }: { searchParams: Promise<{ status?: string; error?: string; success?: string }> }) {
  ...
  return <>
    ...
    <section className="card">
      ...
      <StandardTable><div className="table-wrap shadow-none"><table className="data-table min-w-[1050px]"><thead><tr><th>Homeowner</th><th>Property</th><th>Period</th><th>Reason</th><th>Resolution</th><th>Approved by</th><th>Status</th><th>Created</th><th></th></tr></thead><tbody>{exemptions.map((item) => {
      ...
      })}{!exemptions.length && <tr><td colSpan={9} className="py-12 text-center text-slate-500">No exemptions found.</td></tr>}</tbody></table></div></StandardTable>
    </section>
  </>;
}
