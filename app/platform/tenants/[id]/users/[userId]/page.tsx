...
import { removeTenantUserAction, resetTenantUserPasswordAction, toggleTenantUserAction, updateTenantUserAction } from "@/lib/actions/platform";
...
export default async function TenantUserDetailPage({ params, searchParams }: { params: Promise<{ id: string; userId: string }>; searchParams: Promise<{ success?: string; error?: string; message?: string }> }) {
  ...
  const user = await prisma.user.findFirst({ where: { id: userId, tenantId: id }, include: { tenant: true, homeownerProfile: true, employeeProfile: true } });
  if (!user) notFound();

  return <div className="mx-auto max-w-4xl">
    <Link className="text-sm font-bold text-pine-700" href={`/platform/tenants/${id}/users`}>Back to tenant users</Link>
    <div className="mt-4 rounded-3xl bg-pine-900 p-6 text-white"><p className="text-sm font-bold uppercase tracking-wider text-leaf-100">{user.tenant.name}</p><h1 className="mt-1 text-3xl font-black">{user.name}</h1><p className="mt-2 break-all text-blue-100">{user.email} · {user.active ? "Active" : "Inactive"}</p></div>
    ...
    <form action={updateTenantUserAction} className="mt-5 grid gap-4 rounded-2xl border bg-white p-5 sm:grid-cols-2 sm:p-7">
      ...
      <label><span className="label">Username</span><input className="field" name="username" defaultValue={user.username || ""}/></label>
      <label><span className="label">Role</span><select className="field" name="role" defaultValue={user.role}>{tenantUserRoles.map((role)=><option key={role} value={role}>{roleLabel(role)}</option>)}</select></label>
      ...
      <button className="btn-primary sm:col-span-2">Save User Changes</button>
    </form>
