...
async function authorizedSources(input: {
  ...
  const bindings = await prisma.aiKnowledgeBinding.findMany({
    where: {
      ...
      providerFileId: { in: input.providerFileIds },
      indexStatus: "INDEXED",
    },
  ...
  const documentIds = [...bindingByDocument.keys()];
  const visibility = input.experience === "RESIDENT" ? RepositoryDocumentVisibility.TENANT_PUBLIC : undefined;
  const privacyClassification = input.experience === "RESIDENT" ? "PUBLIC" as const : { in: ["PUBLIC", "INTERNAL"] as const };
...
async function conversationFor(input: { tenantId: string; actorId: string; actorRole: string; retentionDays: number; conversationId?: string | null }) {
  if (input.conversationId) {
    const existing = await prisma.aiConversation.findFirst({
      where: { tenantId: input.tenantId, id: input.conversationId, actorId: input.actorId, status: "ACTIVE", expiresAt: { gt: new Date() } },
    });
...
export async function answerTenantKnowledgeQuestion(input: { experience: AiExperience; question: unknown; conversationId?: string | null }) {
  ...
  let access: Awaited<ReturnType<typeof requireAiRuntimeAccess>>;
  try {
    access = await requireAiRuntimeAccess(input.experience);
  } catch (error) {
    throw error;
  }
  ...
  try {
    ...
    const providerFileIds = providerResponse.citations.map((citation) => citation.fileId);
    const sources = await authorizedSources({ tenantId, experience: input.experience, vectorStoreId: providerIndex.vectorStoreId, providerFileIds, now: new Date() });
