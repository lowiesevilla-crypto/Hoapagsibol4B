...
export default async function BillingPage({ searchParams }: { searchParams: Promise<{ edit?: string }> }) {
  ...
  const [homeowners, bills, archivedBills, editBill, exemptions] = await Promise.all([
    ...
    prisma.duesExemption.findMany({ include: { homeowner: { include: { user: true } }, createdBy: true }, orderBy: [{ billingMonth: "desc" }, { homeowner: { user: { name: "asc" } } }] }),
  ]);
  return <><PageHeader eyebrow="Collections" title="Billing management" description="Generate dues, maintain bill details, and follow up outstanding accounts." action={<form action={sendRemindersAction}><SubmitButton className="btn-secondary"><BellRing className="size-4" /> Send due reminders</SubmitButton></form>} />
    <section className="mb-6 grid gap-5 xl:grid-cols-2">
      <form action={generateMonthlyBillsAction} className="card"><div className="mb-5"><h2 className="text-lg font-black">Generate monthly dues</h2><p className="text-sm text-slate-500">Creates bills for active homeowners, skips duplicates, and automatically excludes approved exemptions.</p></div><div className="grid gap-4 sm:grid-cols-2"><div><label className="label">Billing month</label><input className="field" name="billingMonth" type="month" defaultValue={new Date().toISOString().slice(0, 7)} required /></div><div><label className="label">Due date</label><input className="field" name="dueDate" type="date" required /></div></div><div className="mt-5"><SubmitButton>Generate monthly bills</SubmitButton></div></form>
      <BillForm homeowners={homeowners} bill={editBill ?? undefined} />
    </section>
    <section className="card mb-6"><div className="mb-5 flex items-start gap-3"><span className="grid size-10 place-items-center rounded-xl bg-leaf-50 text-leaf-700"><ShieldCheck className="size-5" /></span><div><h2 className="text-lg font-black">Monthly dues exemptions</h2><p className="text-sm text-slate-500">Approve a specific homeowner for a specific month. The monthly billing generator will skip that account.</p></div></div>
      ...
      <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200"><table className="data-table"><thead><tr><th>Homeowner</th><th>Exempt month</th><th>Reason</th><th>Approved by</th><th></th></tr></thead><tbody>{exemptions.map((item) => <tr key={item.id}><td className="font-bold">{item.homeowner.user.name}<span className="block text-xs font-normal text-slate-400">B{item.homeowner.block} L{item.homeowner.lot}</span></td><td>{monthLabel(item.billingMonth)}</td><td>{item.reason}</td><td>{item.createdBy.name}</td><td className="text-right"><form action={deleteDuesExemptionAction}><input type="hidden" name="id" value={item.id} /><DeleteButton label="Remove" /></form></td></tr>)}{!exemptions.length && <tr><td colSpan={5} className="py-8 text-center text-slate-500">No monthly dues exemptions recorded.</td></tr>}</tbody></table></div>
    </section>
