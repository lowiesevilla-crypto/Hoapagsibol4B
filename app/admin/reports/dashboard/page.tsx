...
function ResponsiveDataTable({ headers, rows, compact = false }: { headers: string[]; rows: string[][]; compact?: boolean }) { return <div className={`${compact ? "mt-5" : "mt-4"} max-w-full overflow-x-auto rounded-lg border border-slate-200`}><StandardTable><table className="w-full table-fixed border-collapse text-left text-[10px] sm:text-sm md:min-w-[580px] md:table-auto"><thead className="bg-slate-50 text-[9px] uppercase text-slate-600 sm:text-xs"><tr>{headers.map((header) => <th className="break-all px-1.5 py-2 sm:break-normal sm:px-3" key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row, rowIndex) => <tr className="border-t border-slate-100" key={rowIndex}>{row.map((cell, cellIndex) => <td className={`break-all px-1.5 py-2 sm:break-normal sm:px-3 ${cellIndex > 0 ? "text-right tabular-nums" : "font-semibold"}`} key={cellIndex}>{cell}</td>)}</tr>)}{!rows.length && <tr><td className="px-3 py-6 text-center text-slate-500" colSpan={headers.length}>No values for this reporting period.</td></tr>}</tbody></table></StandardTable></div>; }
