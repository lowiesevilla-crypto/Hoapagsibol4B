...
const workflowExplanations: Record<DocumentWorkflowPreset, string> = {
  ...
  CUSTOM: "Edit payment, approval, receipt, and delivery rules. Delivery mode and dependent switches stay synchronized so an invalid workflow cannot be saved accidentally.",
};
...
export function DocumentDefinitionWorkflowControls({
  ...
  const updateCustomRules = (patch: Partial<DocumentWorkflowFields>) => setCustomRules((current) => normalizeCustomRules({ ...current, ...patch }));
  const setCustomBoolean = (name: keyof Omit<DocumentWorkflowFields, "deliveryMode">, value: boolean) => updateCustomRules({ [name]: value });

  return <>
    <label>
      <span className="label">Workflow preset</span>
      <select className="field" name="workflowPreset" value={preset} onChange={(event) => {
        const nextPreset = event.currentTarget.value as DocumentWorkflowPreset;
        if (nextPreset === "CUSTOM" && preset !== "CUSTOM") setCustomRules(normalizeCustomRules(rules));
    ...
    <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-semibold text-emerald-950 xl:col-span-3">
      {workflowExplanations[preset]} {locked ? "Related rule fields are preset-controlled. Choose Custom to edit them independently." : "Custom mode persists the synchronized rule fields shown below."}
    </div>
    ...
    <RuleSwitch label="Requires Payment" checked={rules.paymentRequired} disabled={locked || rules.deliveryMode === "REQUEST_ONLY"} onChange={(value) => setCustomBoolean("paymentRequired", value)} />
    <RuleSwitch label="Requires Approval" checked={rules.approvalRequired} disabled={locked || rules.deliveryMode === "REQUEST_ONLY"} onChange={(value) => setCustomBoolean("approvalRequired", value)} />
    <RuleSwitch label="Payment Before Approval" checked={rules.paymentBeforeApproval} disabled={locked || !rules.paymentRequired || !rules.approvalRequired} onChange={(value) => setCustomBoolean("paymentBeforeApproval", value)} />
    <RuleSwitch label="Immediate Download When Eligible" checked={rules.allowImmediateDownload} disabled={locked || rules.paymentRequired || rules.approvalRequired || rules.deliveryMode === "REQUEST_ONLY"} onChange={(value) => setCustomBoolean("allowImmediateDownload", value)} />
    <RuleSwitch label="Requires Admin Review" checked={rules.requiresAdminReview} disabled={locked || !rules.approvalRequired || rules.deliveryMode === "REQUEST_ONLY"} onChange={(value) => setCustomBoolean("requiresAdminReview", value)} />
    <label>
      <span className="label">Delivery mode</span>
      <select className="field" value={rules.deliveryMode} disabled={locked} onChange={(event) => setCustomRules(rulesForDeliveryMode(event.currentTarget.value as DocumentDeliveryMode))}>
      ...
      {!locked && <span className="mt-1 block text-xs font-semibold text-slate-500">Changing delivery mode automatically applies a valid baseline for its payment and approval gates.</span>}
    </label>
    <label>
      ...
      <span className="mt-1 block text-xs font-semibold text-slate-500">{paid ? "The configured fee is snapshotted into each request and must be settled through the linked document-fee payment flow." : "This workflow does not require payment."}</span>
    </label>
    ...
    <label>
      ...
      <input className="field" name="receiptType" value={paid && receiptRequired ? "Document Fee collection receipt" : "Not applicable"} readOnly aria-readonly="true" />
      <span className="mt-1 block text-xs font-semibold text-slate-500">Confirmed document fees are linked to the document request, posted to Collections, and assigned a receipt number.</span>
    </label>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm md:col-span-2 xl:col-span-4">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <Rule label="Immediate Download" value={rules.allowImmediateDownload ? "Yes" : "No"} />
        <Rule label="Payment Timing" value={rules.paymentRequired && rules.approvalRequired ? rules.paymentBeforeApproval ? "Before approval" : "After approval" : rules.paymentRequired ? "Before issuance" : "Not applicable"} />
...
function normalizeCustomRules(rules: DocumentWorkflowFields): DocumentWorkflowFields {
  if (rules.deliveryMode === "REQUEST_ONLY") return rulesForDeliveryMode("REQUEST_ONLY" as DocumentDeliveryMode);
  const paymentRequired = Boolean(rules.paymentRequired);
  const approvalRequired = Boolean(rules.approvalRequired);
  const deliveryMode = paymentRequired && approvalRequired
  ...
  return {
    ...
    paymentRequired,
    approvalRequired,
    paymentBeforeApproval: paymentRequired && approvalRequired ? Boolean(rules.paymentBeforeApproval) : false,
    allowImmediateDownload: !paymentRequired && !approvalRequired,
    requiresAdminReview: approvalRequired ? Boolean(rules.requiresAdminReview) : false,
  };
}

function rulesForDeliveryMode(deliveryMode: DocumentDeliveryMode): DocumentWorkflowFields {
  if (deliveryMode === "INSTANT_DOWNLOAD") return { deliveryMode, paymentRequired: false, approvalRequired: false, paymentBeforeApproval: false, allowImmediateDownload: true, requiresAdminReview: false };
  if (deliveryMode === "APPROVAL_REQUIRED") return { deliveryMode, paymentRequired: false, approvalRequired: true, paymentBeforeApproval: false, allowImmediateDownload: false, requiresAdminReview: true };
  if (deliveryMode === "PAYMENT_REQUIRED") return { deliveryMode, paymentRequired: true, approvalRequired: false, paymentBeforeApproval: false, allowImmediateDownload: false, requiresAdminReview: false };
  if (deliveryMode === "PAYMENT_AND_APPROVAL_REQUIRED") return { deliveryMode, paymentRequired: true, approvalRequired: true, paymentBeforeApproval: true, allowImmediateDownload: false, requiresAdminReview: true };
  return { deliveryMode, paymentRequired: false, approvalRequired: true, paymentBeforeApproval: false, allowImmediateDownload: false, requiresAdminReview: true };
}
