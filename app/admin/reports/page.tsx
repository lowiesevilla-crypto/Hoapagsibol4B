...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  const bondCreditLifetimeRows = await prisma.$queryRaw<TotalRow[]>(Prisma.sql`SELECT COALESCE(SUM(amount),0) AS total FROM Payment WHERE tenantId=${user.tenantId} AND status='ACTIVE' AND paymentBatchId LIKE ${`${BOND_DUES_CREDIT_PREFIX}%`}`);
  ...
  const cashOutflows = operatingExpenses + payrollCashDisbursements + bondsRefunded + employeeLoansIssued;
  ...
  const bondsHeld = Math.max(0, Number(allBondTotals._sum.amount ?? 0) - Number(allBondTotals._sum.amountRefunded ?? 0) - Number(allBondTotals._sum.amountForfeited ?? 0) - lifetimeBondCreditsAppliedToDues);
  ...
  return <>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <div className="mb-4">
        <h2 className="font-black text-slate-900">Financial position at a glance</h2>
        <p className="text-sm text-slate-500">Summary values are derived from the same tenant-scoped accounting data and calculations shown below.</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="Recognized income" value={money(recognizedIncome)} note="Dues, fees and forfeitures" icon={ReceiptText} /><StatCard label="Operating expenses" value={money(totalExpenses)} note="Expenses plus finalized payroll" icon={WalletCards} /><StatCard label="Operating surplus" value={money(operatingSurplus)} note="Income less operating expenses" icon={TrendingUp} /><StatCard label="Refundable bonds held" value={money(bondsHeld)} note="Ending liability after refunds, forfeitures and dues credits" icon={HandCoins} /><StatCard label="Employee loan balance" value={money(employeeLoanOutstanding)} note="Cash advances and loans receivable" icon={HandCoins} /></div>
    </section>

    <section className="mt-6">
      <div className="mb-3">
        <h2 className="font-black text-slate-900">Core financial statements</h2>
        <p className="text-sm text-slate-500">Income recognition, liabilities, cash movement, payroll, and expense treatment remain governed by the existing finance logic.</p>
      </div>
      <div className="grid gap-6 xl:grid-cols-2"><Statement title="Statement of Income and Expenses" subtitle={`${shortDate(from)} to ${shortDate(to)}`}><SectionLabel text="REVENUE" /><Row label="Homeowner monthly dues" value={duesIncome} />{[...feeBreakdown.entries()].map(([label, value]) => <Row key={label} label={label} value={value} />)}<Row label="Forfeited bond income" value={forfeitedIncome} /><Row label="Total revenue" value={recognizedIncome} total /><SectionLabel text="OPERATING EXPENSES" />{[...expenseBreakdown.entries()].map(([label, value]) => <Row key={label} label={label} value={value} />)}<Row label="Employee payroll" value={payrollExpense} /><Row label="Total operating expenses" value={totalExpenses} total /><Row label="NET OPERATING SURPLUS / (DEFICIT)" value={operatingSurplus} grand /></Statement>
        <Statement title="Statement of Cash Receipts and Disbursements" subtitle={`${shortDate(from)} to ${shortDate(to)}`}><SectionLabel text="CASH RECEIPTS" /><Row label="Monthly dues cash received" value={paymentCashReceived} /><Row label="Other fee collections" value={feeIncome} /><Row label="Rental security deposits received (liability)" value={rentalSecurityDepositsReceived} /><Row label="Refundable bonds received" value={bondsReceived} /><Row label="Total cash receipts" value={cashInflows} total /><SectionLabel text="NON-CASH / PAYMENT ALLOCATION MEMORANDUM" /><Row label="Construction Bond applied to Monthly Dues (non-cash)" value={bondCreditsAppliedToDues} /><Row label="Total amount applied to dues" value={duesIncome} /><Row label="Unapplied homeowner credits" value={unappliedCredits} /><SectionLabel text="CASH DISBURSEMENTS" /><Row label="Operating expenses" value={operatingExpenses} /><Row label="Employee payroll" value={payrollCashDisbursements} /><Row label="Employee loans / cash advances issued" value={employeeLoansIssued} /><Row label="Bond refunds" value={bondsRefunded} /><Row label="Total cash disbursements" value={cashOutflows} total /><Row label="NET CASH MOVEMENT" value={netCashMovement} grand /></Statement></div>
    </section>

    <section className="card mt-6"><h2 className="text-lg font-black">Monthly dues collection summary</h2><p className="mb-4 text-sm text-slate-500">Receipt-level cash, non-cash bond credits, applied dues, and unapplied homeowner credit.</p><div className="table-wrap shadow-none"><table className="data-table"><thead><tr><th>Receipt</th><th>Homeowner</th><th>Payment date</th><th>Payment Coverage</th><th>Source</th><th className="text-right">Cash received</th><th className="text-right">Applied</th><th className="text-right">Credit</th></tr></thead><tbody>{payments.map((payment) => { const bondCredit = isBondDuesCreditPayment(payment); return <tr key={payment.id}><td className="font-mono text-xs font-bold text-pine-700">{payment.receiptNumber || "Legacy receipt"}</td><td className="font-bold">{payment.homeowner.user.name}</td><td>{shortDate(payment.paymentDate)}</td><td>{paymentAllocationCoverageLabel(payment)}</td><td>{bondCredit ? "Construction Bond Credit" : "Cash / Payment"}</td><td className="text-right font-black">{money(bondCredit ? 0 : Number(payment.amount))}</td><td className="text-right">{money(paymentAppliedAmount(payment))}</td><td className="text-right">{money(bondCredit ? 0 : paymentUnappliedCredit(payment))}</td></tr>; })}{!payments.length && <tr><td colSpan={8} className="py-10 text-center text-slate-500">No monthly dues collections or bond credits in this reporting period.</td></tr>}</tbody></table></div></section>

    <section className="card mt-6"><h2 className="text-lg font-black">Refundable bond accountability</h2><p className="mb-4 text-sm text-slate-500">Bonds remain liabilities until returned, forfeited, or applied with homeowner authorization to Monthly Dues.</p><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Bonds received this period" value={bondsReceived} /><Metric label="Bonds refunded this period" value={bondsRefunded} /><Metric label="Applied to Monthly Dues this period" value={bondCreditsAppliedToDues} /><Metric label="Bonds forfeited to income" value={forfeitedIncome} /><Metric label="Ending bonds held" value={bondsHeld} /></div></section>
