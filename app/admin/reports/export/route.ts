...
export async function GET() {
  ...
  const rows = [
    ...
    ...collections.map((item) => [item.id, item.receiptNumber ?? "", "Collection", collectionLabel(item.type, item.description), collectionPayerName(item), item.collectionDate.toISOString().slice(0, 10), item.method, item.referenceNumber ?? "", item.amount.toString(), "", item.refundable ? "Refundable liability" : "Income"]),
