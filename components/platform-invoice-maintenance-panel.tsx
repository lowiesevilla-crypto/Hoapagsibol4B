...
import { cancelPlatformInvoiceAction } from "@/lib/actions/platform-billing";
import { updatePlatformInvoiceAction } from "@/lib/actions/platform-invoice-maintenance";
...
export async function PlatformInvoiceMaintenancePanel({ tenantId }: { tenantId: string }) {
  ...
  return <details id="invoice-maintenance" open className="mt-4 scroll-mt-4 rounded-2xl border-2 border-sky-300 bg-sky-50/70 p-4 shadow-sm">
    ...
    <p className="mt-1 text-xs text-slate-500">Edit and delete are limited to invoices without payment history. Delete removes an eligible draft/open/overdue invoice from active billing while retaining cancelled audit evidence and preserving the subscription schedule.</p>
    <div className="mt-4 overflow-auto rounded-xl border bg-white">
      <table className="min-w-[1120px] w-full text-sm">
        ...
        <tbody>{invoices.map((invoice) => {
          ...
          const deletable = noPaymentHistory && [PlatformInvoiceStatus.DRAFT, PlatformInvoiceStatus.OPEN, PlatformInvoiceStatus.OVERDUE].includes(invoice.status);
          ...
          return <tr key={invoice.id} className="border-t align-top">
            ...
            <td className="p-3">
              {editable ? <div className="flex min-w-80 flex-col gap-2">
                ...
                {deletable ? <form action={cancelPlatformInvoiceAction}>
                  <input type="hidden" name="tenantId" value={tenantId} />
                  <input type="hidden" name="invoiceId" value={invoice.id} />
                  <DeleteButton label="Delete invoice" />
                </form> : <span className="text-[11px] font-semibold text-slate-400">Delete protected · payment/history or closed status</span>}
