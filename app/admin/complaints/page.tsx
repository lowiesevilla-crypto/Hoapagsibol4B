...
export default async function AdminComplaintsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <>
    ...
    <form className={`card mb-5 grid gap-3 ${canManageGrievance ? "md:grid-cols-2 xl:grid-cols-[1fr_190px_190px_210px_210px_auto]" : "md:grid-cols-[1fr_210px_210px_auto]"}`} method="get">
      <div><label className="sr-only" htmlFor="complaint-search">Search complaints</label><input id="complaint-search" className="field" type="search" name="q" defaultValue={query.q || ""} placeholder="Search number, reference, title, or location" /></div>
      <div><label className="sr-only" htmlFor="complaint-status-filter">Complaint status</label><select id="complaint-status-filter" className="field" name="status" defaultValue={query.status || ""}><option value="">All complaint statuses</option>{Object.values(ComplaintStatus).map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></div>
      <div><label className="sr-only" htmlFor="complaint-privacy-filter">Privacy mode</label><select id="complaint-privacy-filter" className="field" name="privacy" defaultValue={query.privacy || ""}><option value="">All privacy modes</option>{Object.values(ComplaintPrivacyMode).map((mode) => <option key={mode} value={mode}>{complaintPrivacyLabel(mode)}</option>)}</select></div>
      {canManageGrievance && <div><label className="sr-only" htmlFor="complaint-grievance-filter">Grievance status</label><select id="complaint-grievance-filter" className="field" name="grievanceStatus" defaultValue={query.grievanceStatus || ""}><option value="">All grievance states</option>{grievanceStatuses.map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></div>}
      {canManageGrievance && <div><label className="sr-only" htmlFor="complaint-verification-filter">Verification status</label><select id="complaint-verification-filter" className="field" name="verificationStatus" defaultValue={query.verificationStatus || ""}><option value="">All verification states</option>{verificationStatuses.map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></div>}
