...
import { ComplaintStatus, ComplaintVisibility } from "@prisma/client";
...
import { complaintAdminRoles, complaintPrivacyLabel, complaintStatusLabel, getAdminComplaintDetail, requireComplaintAdmin } from "@/lib/services/complaints";
...
export default async function AdminComplaintDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const [complaint, assignees] = await Promise.all([
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
      ...
      <aside className="space-y-5">
        <section className="card"><h2 className="text-lg font-black">Case Controls</h2>
          <form action={updateComplaintStatusAction} className="mt-4 space-y-3"><input type="hidden" name="id" value={complaint.id} /><select className="field" name="status" defaultValue={complaint.status}>{Object.values(ComplaintStatus).map((status) => <option key={status} value={status}>{complaintStatusLabel(status)}</option>)}</select><textarea className="field min-h-20" name="note" placeholder="Status note" /><button className="btn-primary w-full">Update status</button></form>
          <form action={assignComplaintAction} className="mt-5 space-y-3"><input type="hidden" name="id" value={complaint.id} /><select className="field" name="assigneeId" defaultValue={complaint.assignedToId || ""} required><option value="">Assign handler</option>{assignees.map((assignee) => <option key={assignee.id} value={assignee.id}>{assignee.name} ({assignee.role.replaceAll("_", " ")})</option>)}</select><button className="btn-secondary w-full">Assign</button></form>
        </section>
