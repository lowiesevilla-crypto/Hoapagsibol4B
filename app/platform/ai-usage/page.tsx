...
export default async function PlatformAiUsagePage() {
  ...
  const [monthUsage, successfulRequests, blockedRequests, tenantUsage, recent] = await Promise.all([
    ...
    prisma.aiUsageLedger.groupBy({ by: ["tenantId"], where: { createdAt: { gte: monthStart } }, _count: { _all: true }, _sum: { inputTokens: true, outputTokens: true, estimatedCostCentavos: true }, orderBy: { _count: { tenantId: "desc" } }, take: 12 }),
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_440px]">
      <WorkspaceCard title="Tenant AI adoption" description="Highest request volume for the current month."><div className="divide-y divide-slate-100">{tenantUsage.map((row) => <div key={row.tenantId} className="grid gap-2 py-3 first:pt-0 last:pb-0 sm:grid-cols-[minmax(0,1fr)_100px_150px_120px] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-900">{tenantName.get(row.tenantId) ?? "Tenant"}</p><p className="mt-1 truncate font-mono text-[10px] text-slate-400">{row.tenantId}</p></div><p className="text-sm font-black text-platform-700">{row._count._all} requests</p><p className="text-xs font-bold text-slate-500">{(Number(row._sum.inputTokens ?? 0) + Number(row._sum.outputTokens ?? 0)).toLocaleString("en-PH")} tokens</p><p className="text-right text-sm font-black text-pine-900">{moneyCentavos(Number(row._sum.estimatedCostCentavos ?? 0))}</p></div>)}{!tenantUsage.length ? <p className="py-10 text-center text-sm text-slate-500">No AI usage recorded this month.</p> : null}</div></WorkspaceCard>
      <WorkspaceCard title="Recent AI requests" description="Outcome and performance metadata without prompt/response content."><div className="divide-y divide-slate-100">{recent.map((row) => <div key={row.id} className="py-3 first:pt-0 last:pb-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-900">{tenantName.get(row.tenantId) ?? "Tenant"}</p><p className="mt-1 text-xs text-slate-500">{row.model || "Provider model"} · {row.inputTokens + row.outputTokens} tokens · {row.latencyMs ? `${row.latencyMs} ms` : "latency unavailable"}</p></div><StatusBadge tone={row.outcome === AiRequestOutcome.SUCCEEDED ? "success" : row.outcome === AiRequestOutcome.PROVIDER_ERROR ? "critical" : "warning"}>{row.outcome.replaceAll("_", " ")}</StatusBadge></div></div>)}{!recent.length ? <p className="py-10 text-center text-sm text-slate-500">No AI requests recorded.</p> : null}</div></WorkspaceCard>
    </section>
  </div>;
}
