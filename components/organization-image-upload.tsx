...
export function OrganizationImageUpload({ kind, currentUrl }: { kind: "photo" | "signature"; currentUrl?: string | null }) {
  ...
  function select(file?: File) {
    ...
    if (![".jpg", ".jpeg", ".png"].includes(extension) || !allowed.includes(file.type)) { setError("Select a JPG, JPEG, or PNG image."); setName(""); return; }
  ...
  return <div className="rounded-2xl border border-slate-200 p-3">
    ...
    <input id={`organization-${kind}`} className="field" type="file" name={kind} accept=".jpg,.jpeg,.png" onChange={(event) => select(event.target.files?.[0])} />
