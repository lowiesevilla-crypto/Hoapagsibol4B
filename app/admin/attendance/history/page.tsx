...
export default async function AttendanceHistoryPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const [employees, rawRecords, periods] = await Promise.all([
    ...
    prisma.payrollPeriod.findMany({ include: { payslips: { select: { employeeId: true } } }, orderBy: { endDate: "desc" } }),
  ]);
  ...
  return <>
    ...
    <form className="card mb-6" method="get">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <div><label className="label">Payroll status</label><select className="field" name="payrollStatus" defaultValue={params.payrollStatus ?? "ALL"}><option value="ALL">All</option><option value="NOT_INCLUDED">Not included</option><option value="DRAFT">Draft</option><option value="FINALIZED">Finalized</option><option value="PAID">Paid</option></select></div>
      </div>
    ...
    <div className="table-wrap"><StandardTable><table className="data-table"><thead><tr><th>Date</th><th>Employee</th><th>Status</th><th>Payroll</th><th>Time</th><th>Total</th><th>Late / undertime</th><th>Remarks</th><th>Actions</th></tr></thead><tbody>{records.map((record) => {
    ...
    })}{!records.length && <tr><td colSpan={9} className="py-12 text-center text-slate-500">No attendance records match these filters.</td></tr>}</tbody></table></StandardTable></div>
  </>;
}
