...
export default async function TenantLoginPage({ params, searchParams }: { params: Promise<{ tenantSlug: string }>; searchParams: Promise<{ reset?: string }> }) {
  const { tenantSlug } = await params; const query = await searchParams; const tenant = await resolveTenant(tenantSlug); if (!tenant) notFound();
