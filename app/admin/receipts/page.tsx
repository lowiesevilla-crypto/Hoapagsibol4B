...
export default async function ReceiptRegisterPage({ searchParams }: { searchParams: Promise<{ q?: string; series?: string }> }) {
  ...
  const [payments, collections] = await Promise.all([
    prisma.payment.findMany({ where: { tenantId: admin.tenantId, ...(prefix ? { receiptNumber: { contains: prefix } } : q ? { OR: [{ receiptNumber: { contains: q } }, { referenceNumber: { contains: q } }, { homeowner: { user: { name: { contains: q } } } }] } : {}) }, include: { homeowner: { include: { user: true } }, bill: true, allocations: { include: { bill: true }, orderBy: { bill: { billingMonth: "asc" } } }, processedBy: { include: { employeeProfile: true } } }, orderBy: { createdAt: "desc" }, take: 200 }),
    prisma.collection.findMany({ where: { tenantId: admin.tenantId, ...(prefix ? { receiptNumber: { contains: prefix } } : q ? { OR: [{ receiptNumber: { contains: q } }, { referenceNumber: { contains: q } }, { homeowner: { user: { name: { contains: q } } } }, { contractor: { companyName: { contains: q } } }] } : {}) }, include: { homeowner: { include: { user: true } }, contractor: true, createdBy: true }, orderBy: { createdAt: "desc" }, take: 200 }),
  ]);
  const rows = [
    ...payments.map((item) => ({ id: item.id, kind: "payment", receipt: item.receiptNumber, series: "MD", date: item.paymentDate, payer: item.homeowner.user.name, purpose: `${paymentAllocationCoverageDisplay(item)} | Applied ${money(paymentAppliedAmount(item))} | Credit ${money(paymentUnappliedCredit(item))}`, amount: item.amount, reference: item.referenceNumber, processor: paymentProcessorIdentity(item.processedBy).name, status: item.status === "VOIDED" ? "Void" : "Active" })),
    ...collections.map((item) => ({ id: item.id, kind: "collection", receipt: item.receiptNumber, series: item.type === "CONSTRUCTION_BOND" ? "CB" : item.type === "CONTRACTOR_BOND" ? "CTB" : "OC", date: item.collectionDate, payer: item.homeowner?.user.name ?? item.contractor?.companyName ?? "Unknown", purpose: collectionLabel(item.type, item.description), amount: item.amount, reference: item.referenceNumber, processor: item.createdBy.name, status: "Active" })),
  ...
  return <>
    ...
    <StandardTable><div className="table-wrap"><table className="data-table"><thead><tr><th>Receipt number</th><th>Date</th><th>Payer</th><th>Particulars</th><th>Reference</th><th>Processed by</th><th>Status</th><th className="text-right">Amount</th><th></th></tr></thead><tbody>{rows.map((item) => <tr key={`${item.kind}-${item.id}`}><td className="font-mono text-xs font-black text-pine-700">{item.receipt || "Legacy receipt pending"}</td><td>{shortDate(item.date)}</td><td className="font-bold">{item.payer}</td><td>{item.purpose}</td><td className="font-mono text-xs">{item.reference || "-"}</td><td>{item.processor}</td><td className={item.status === "Void" ? "font-black text-rose-700" : "font-bold text-emerald-700"}>{item.status}</td><td className="text-right font-black">{money(item.amount)}</td><td><Link className="btn-secondary min-h-8 px-3 py-1 text-xs" href={`/receipts/${item.kind}/${item.id}`} target="_blank">View / Print</Link></td></tr>)}{rows.length === 0 && <tr><td colSpan={9} className="py-12 text-center text-slate-500">No receipts match the selected search.</td></tr>}</tbody></table></div></StandardTable>
  </>;
}
