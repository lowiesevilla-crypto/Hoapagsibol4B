...
export default async function VerifyDocumentPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const request = await platformPrisma.documentRequest.findUnique({ where: { verificationCode: code.toUpperCase() }, include: { homeowner: { include: { user: true } }, definition: true, configuration: true, processedBy: true, approvedBy: true } });
