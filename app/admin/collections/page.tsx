...
export default async function CollectionsPage({ searchParams }: CollectionsPageProps) {
  ...
  const bondsHeld = collections.filter((item) => isRefundableBondType(item.type)).reduce((sum, item) => sum + Number(item.amount) - Number(item.amountRefunded) - Number(item.amountForfeited), 0);
  const openBonds = collections.filter((item) => {
    ...
    return status !== RefundStatus.REFUNDED && status !== RefundStatus.FORFEITED && Number(item.amount) - Number(item.amountRefunded) - Number(item.amountForfeited) > 0;
  });
  ...
  return <><PageHeader eyebrow="Income and liabilities" title="Other collections & bonds" description="Record association income separately from refundable homeowner and contractor bonds." />
    ...
    <div className="table-wrap"><table className="data-table"><thead><tr><th>Payer</th><th>Collection</th><th>Date / method</th><th>Amount</th><th>Refunded</th><th>Balance held</th><th>Status</th><th></th></tr></thead><tbody>{collections.map((item) => { const isBond = isRefundableBondType(item.type); const balance = isBond ? Number(item.amount) - Number(item.amountRefunded) - Number(item.amountForfeited) : 0; const status = isBond ? effectiveBondRefundStatus(item.type, item.refundStatus) : "INCOME"; const payer = payerInfo(item); const hasFinancialHistory = item.refunds.length > 0 || Number(item.amountForfeited) > 0; return <tr key={item.id} data-search={`${payer.name} ${payer.category} ${item.type} ${status} ${item.referenceNumber ?? ""}`.toLowerCase()}><td><p className="font-bold">{payer.name}</p><p className="text-xs text-slate-400">{payer.category.toLowerCase().replaceAll("_", " ")}</p></td><td><p className="font-bold">{collectionLabel(item.type, item.description)}</p><p className="text-xs text-slate-400">{isBond ? "Refundable bond" : "Association income"}</p></td><td><p>{shortDate(item.collectionDate)}</p><p className="text-xs text-slate-400">{item.method.replaceAll("_", " ")}</p></td><td className="font-black">{money(item.amount)}</td><td>{money(item.amountRefunded)}</td><td>{money(balance)}</td><td><StatusBadge status={status} /></td><td><div className="flex min-w-32 flex-col gap-2"><Link className="btn-secondary min-h-8 px-3 py-1" href={`/receipts/collection/${item.id}`} target="_blank"><Printer className="size-4" /> Receipt</Link>{isBond && balance > 0 && <details><summary className="cursor-pointer text-xs font-bold text-rose-700">Forfeit balance</summary><form action={forfeitBondAction} className="mt-2 space-y-2"><input type="hidden" name="collectionId" value={item.id} /><input className="field min-w-44" name="reason" placeholder="Violation reason" required /><SubmitButton className="btn-danger w-full">Confirm forfeiture</SubmitButton></form></details>}{hasFinancialHistory ? <p className="px-1 text-center text-xs font-bold text-slate-500" title="Refunded and forfeited bonds are retained for audit integrity.">Retained for audit</p> : <form action={deleteCollectionAction}><input type="hidden" name="id" value={item.id} /><DeleteButton /></form>}</div></td></tr>; })}{!collections.length && <tr><td colSpan={8} className="py-12 text-center text-slate-500">No other collections have been recorded.</td></tr>}</tbody></table></div>
