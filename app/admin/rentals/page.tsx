...
type RenterRow = { id: string; fullName: string; email: string | null; phone: string | null; address: string | null; status: string; homeownerId: string | null; homeownerName: string | null; agreementCount: bigint | number };
...
type CollectionRow = { id: string; receiptNumber: string; payerType: string; payerName: string | null; homeownerId: string | null; homeownerName: string | null; amount: Prisma.Decimal | number | string; allocated: Prisma.Decimal | number | string; available: Prisma.Decimal | number | string };
...
export default async function RentalsPage() {
  ...
  const [assets, renters, agreements, invoices, collections, homeowners] = await Promise.all([
    ...
    prisma.$queryRaw<CollectionRow[]>(Prisma.sql`
      SELECT c.id,c.receiptNumber,c.payerType,c.payerName,c.homeownerId,u.name AS homeownerName,c.amount,
      ...
      GROUP BY c.id,c.receiptNumber,c.payerType,c.payerName,c.homeownerId,u.name,c.amount HAVING available>0 ORDER BY c.collectionDate DESC LIMIT 5000
    `),
  ...
  return <>
    ...
    <section className="mb-6 grid gap-5 xl:grid-cols-3">
      ...
      <form action={saveRenterAction} className="card space-y-3">
        ...
        <SearchableHomeownerSelect name="homeownerId" label="Existing homeowner link (optional)" homeowners={homeownerOptions} searchEndpoint="/api/admin/homeowners/search" placeholder="Search homeowner name, block, lot, account, or email" />
