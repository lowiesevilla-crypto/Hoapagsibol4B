...
export function TenantLoginScreen({ tenant, reset, loggedOut, returnTo, universal = false }: { tenant: LoginTenant; reset?: string; loggedOut?: string; returnTo?: string; universal?: boolean }) {
  const features = [
  ...
  return <main className={`${styles.shell} grid overflow-x-hidden lg:grid-cols-[1.08fr_.92fr]`}>
    <section className={`${styles.hero} brand-hero brand-grid relative max-lg:!hidden min-h-screen flex-col justify-between overflow-hidden p-10 text-white lg:!flex xl:p-16`}>
      <div className={styles.heroGlow} aria-hidden="true" />
      <div className={styles.heroGlowAlt} aria-hidden="true" />
      ...
      <div className="relative max-w-2xl py-10">
        <p className={`${styles.badgeEntrance} mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-pine-900/35 px-4 py-2 text-xs font-extrabold uppercase tracking-[.2em] text-white`}>
          <CheckCircle2 className="size-4 text-leaf-100" /> Growing together, managed clearly
        </p>
        <h1 className={`${styles.titleEntrance} max-w-xl text-5xl font-black leading-[1.06] text-white xl:text-6xl`}>
          One secure portal for a <span className="text-[#dff8d2]">connected community.</span>
        </h1>
        <p className={`${styles.copyEntrance} mt-6 max-w-xl text-lg leading-8 text-[#d7effb]`}>
          Access association finances, resident services, official notices, and community activities with clarity and confidence.
        </p>
        <ul className={`${styles.featureGrid} mt-9 grid max-w-2xl gap-3 sm:grid-cols-2`}>
          {features.map((item) => {
            const Icon = item.icon;
            return <li className="flex items-center gap-3 rounded-2xl border border-white/20 bg-pine-900/45 p-4 shadow-lg" key={item.title}>
              <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-leaf-100 text-pine-900"><Icon className="size-5" /></span>
              <span>
                <span className="block text-sm font-black text-white">{item.title}</span>
                <span className="mt-0.5 block text-xs leading-5 text-[#d7effb]">{item.note}</span>
              </span>
            </li>;
          })}
        </ul>
      ...
      <p className={`${styles.secureEntrance} relative flex items-center gap-2 text-sm font-semibold text-[#d7effb]`}>
        <LockKeyhole className="size-4 text-leaf-100" /> Secure access for association officers and residents
      </p>
    </section>
    ...
    <section className={`${styles.loginStage} relative grid place-items-center overflow-hidden px-4 py-4 sm:p-10`}>
      <div className={`${styles.loginCard} relative w-full max-w-md overflow-hidden rounded-[2rem] border border-pine-100 bg-white p-5 shadow-[0_24px_70px_rgba(8,97,141,.16)] sm:p-10`}>
        ...
        {loggedOut === "1" && <p className="mb-4 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800 sm:mb-5">You have been signed out securely.</p>}
        {reset === "success" && <p className="mb-4 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800 sm:mb-5">Password updated successfully.</p>}
        {returnTo && <p className="mb-4 rounded-xl bg-blue-50 p-3 text-sm font-semibold text-blue-900">Sign in to continue to the secure link you opened.</p>}
